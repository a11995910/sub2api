...
GITHUB_API_CURL_SOURCE=$(sed -n '/^github_api_curl() {$/,/^}$/p' "$ROOT_DIR/deploy/install.sh")
test -n "$GITHUB_API_CURL_SOURCE"
...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        bash -c 'source /dev/stdin <<< "$1"; github_api_curl -s "$2"' bash \
...
assert_unsafe_invocation_rejected() {
    ...
    if CURL_ARGS_LOG="$TEMP_DIR/$name" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="update-secret" \
        bash -c 'source /dev/stdin <<< "$1"; shift; github_api_curl "$@"' bash \
