...
| `GEMINI_CLI_OAUTH_CLIENT_SECRET` | Required for built-in flow | *(empty)* | Gemini CLI built-in OAuth client secret, injected at runtime. |
...
Code Assist always uses the Gemini CLI built-in OAuth client, but this repository does not embed that client ID/secret. Inject them with environment variables in the runtime environment.

1. Set `GEMINI_CLI_OAUTH_CLIENT_ID` and `GEMINI_CLI_OAUTH_CLIENT_SECRET`
2. Leave `GEMINI_OAUTH_CLIENT_ID` and `GEMINI_OAUTH_CLIENT_SECRET` empty
3. In the Admin UI, create a Gemini OAuth account and select **"Code Assist"** type
...
# 如需使用 Gemini CLI 内置 OAuth Client（Code Assist / Google One）
# 安全说明：本仓库不会内置该 client_id/client_secret，请在运行环境通过环境变量注入。
GEMINI_CLI_OAUTH_CLIENT_ID=your-built-in-client-id
GEMINI_CLI_OAUTH_CLIENT_SECRET=your-built-in-client-secret
```
...
2. Add your OAuth credentials in the `[Service]` section (after the existing `Environment=` lines):
   ...
   ```ini
   Environment=GEMINI_CLI_OAUTH_CLIENT_ID=your-built-in-client-id
   Environment=GEMINI_CLI_OAUTH_CLIENT_SECRET=your-built-in-client-secret
   ```
