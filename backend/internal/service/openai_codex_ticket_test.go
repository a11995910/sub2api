...
func ticketTestAccount(id int64) *Account {
	return &Account{
		...
		Extra:       map[string]any{"openai_turn_state_mode": "codex_ticket"},
	}
}
...
func TestLookupOpenAICodexTicket_HydratesFromExtra(t *testing.T) {
	...
	account.Extra = map[string]any{"openai_turn_state_mode": "codex_ticket",
...
// 两个默认模型并发完成刷新，用竞态检测验证账号映射没有共享写入。
...
func TestRefreshOpenAICodexTickets_ConcurrentModelsPreserveAccountSnapshot(t *testing.T) {
	...
	account.Extra = map[string]any{"openai_turn_state_mode": "codex_ticket", "existing": true}
...
func TestCodexTicketModeIsolationAndProbeBypass(t *testing.T) {
	...
	account.Extra["openai_turn_state_mode"] = "codex_ticket"
	h := http.Header{}
	h.Set(openAICodexTurnStateHeader, "probe-state")
	require.NoError(t, svc.applyOpenAICodexTicket(WithOpenAITurnStateProbe(context.Background()), account, "gpt-6-astra", h))
	require.Equal(t, "probe-state", h.Get(openAICodexTurnStateHeader))
}

func TestCodexTicketAccountFailClosedOverridesGlobal(t *testing.T) {
	for _, fallback := range []bool{false, true} {
		svc := ticketTestService(t, config.OpenAICodexTicketConfig{Enabled: true, FailClosed: fallback}, nil)
		account := ticketTestAccount(41)
		for _, value := range []bool{false, true} {
			account.Extra["openai_codex_ticket_fail_closed"] = value
			h := http.Header{}
			h.Set(openAICodexTurnStateHeader, "client-state")
			err := svc.applyOpenAICodexTicket(context.Background(), account, "gpt-6-astra", h)
			require.Equal(t, value, svc.openAICodexTicketBlocksAccount(account, "gpt-6-astra"))
			statuses := OpenAICodexTicketStatuses(account, svc.openAICodexTicketConfig(), time.Now())
			require.Equal(t, value, statuses[0].Blocked)
			if value {
				require.ErrorIs(t, err, ErrOpenAICodexTicketUnavailable)
			} else {
				require.NoError(t, err)
	...
	require.Error(t, ValidateOpenAICodexTicketExtra(map[string]any{"openai_codex_ticket_fail_closed": "true"}))
}
	...
	for _, phase := range []string{"发送前", "保存前"} {
		for _, mode := range []string{OpenAITurnStateOff, "healthy_retry", "healthy_preflight"} {
			t.Run(phase+"/"+mode, func(t *testing.T) {
				...
				repo := &codexTicketRefreshRepo{accounts: []Account{latest}}
				var calls int
				changeMode := func() { repo.mu.Lock(); repo.accounts[0].Extra[OpenAITurnStateModeKey] = mode; repo.mu.Unlock() }
				upstream := &codexTicketFuncUpstream{do: func(*http.Request) (*http.Response, error) { calls++; changeMode(); return codexTicketResponse(), nil }}
				svc := ticketTestService(t, config.OpenAICodexTicketConfig{Enabled: true, HarvestProxyURL: "http://proxy.example.com:8080"}, upstream)
				svc.accountRepo = repo
				if phase == "发送前" {
					changeMode()
...
func TestCodexTicketProbeDropsResultAfterGlobalSwitchOff(t *testing.T) {
	account := ticketTestAccount(942)
	account.Status = StatusActive
	var enabled atomic.Bool
	enabled.Store(true)
	settings := &codexTicketLifecycleSettings{get: func(_ context.Context, key string) (string, error) {
	...
	svc := ticketTestService(t, config.OpenAICodexTicketConfig{Enabled: true, HarvestProxyURL: "http://proxy.example.com:8080"}, nil)
