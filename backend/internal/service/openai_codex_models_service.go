...
// loadCodexGroupCatalogAccounts 将当前可选账号与能力交集账号分开：visible 账号决定
// 展示哪些公共别名，catalog 账号则包含分组内持久启用的成员。能力查询忽略限流、过载
// 和临时不可调度状态，避免瞬态状态扩大对外声明的能力；持久禁用账号因路由不可选而排除。
// 查询失败时回退当前可调度集合，避免列表错误直接导致客户端请求失败。
...
func configuredCodexSupportsPriorityServiceTier(modelID string) bool {
	_, ok := resolveOpenAIFastModelPolicy(modelID)
	return ok
}
...
func accountCodexModelSupportsImageInput(account *Account, upstreamModel string) bool {
	...
	case PlatformOpenAI:
		...
		// 兼容上游的模型列表经常省略模态信息；除非已同步快照明确收窄能力，
		// 否则保留已知 GPT 图片输入兜底。
		return true
	case PlatformGrok:
...
func completeAPIKeyCodexModelsManifestMetadata(body []byte, completeAll bool, account *Account, synthesizeServiceTiers bool) ([]byte, error) {
	var envelope map[string]json.RawMessage
	if err := json.Unmarshal(body, &envelope); err != nil {
		return nil, fmt.Errorf("decode JSON object: %w", err)
	}
	...
	for i, rawModel := range models {
		...
		// 标准 /models 列表不携带 Codex 能力元数据，可使用本地官方 SKU 表补全。
		// Provider 原生 Codex manifest 则以其能力声明为准，缺失时不得猜测 Fast。
		if !synthesizeServiceTiers {
