...
func TestBillingService_GPT56UsesPublishedShortAndLongPricingAcrossModelsAndTiers(t *testing.T) {
	models := []struct {
		name               string
	...
	contexts := []struct {
		...
		longContextBillingIsExpected bool
	}{
		{name: "short", cacheReadTokens: 72000, inputScale: 1, outputScale: 1},
		{name: "long", cacheReadTokens: 72001, inputScale: 2, outputScale: 1.5, longContextBillingIsExpected: true},
	}
	svc := NewBillingService(&config.Config{}, newStubPricingServiceFromJSON(t, gpt56LadderCatalogJSON))

	for _, model := range models {
		for _, contextTier := range contexts {
			for _, serviceTier := range tiers {
				t.Run(model.name+"/"+contextTier.name+"/"+serviceTier.name, func(t *testing.T) {
					tokens := UsageTokens{
						InputTokens:         100000,
						CacheCreationTokens: 100000,
						CacheReadTokens:     contextTier.cacheReadTokens,
						OutputTokens:        10,
					}
					tier := ""
					if serviceTier.name != "standard" {
						tier = serviceTier.name
					}
					cost, err := svc.CalculateCostWithServiceTier(model.name, tokens, 1, tier)
					require.NoError(t, err)
					require.InDelta(t, float64(tokens.InputTokens)*model.input*serviceTier.priceScale*contextTier.inputScale, cost.InputCost, 1e-12)
					require.InDelta(t, float64(tokens.CacheCreationTokens)*model.cacheWrite*serviceTier.priceScale*contextTier.inputScale, cost.CacheCreationCost, 1e-12)
					require.InDelta(t, float64(tokens.CacheReadTokens)*model.cached*serviceTier.priceScale*contextTier.inputScale, cost.CacheReadCost, 1e-12)
					require.InDelta(t, float64(tokens.OutputTokens)*model.output*serviceTier.priceScale*contextTier.outputScale, cost.OutputCost, 1e-12)
					require.Equal(t, contextTier.longContextBillingIsExpected, cost.LongContextBillingApplied)
				})
			}
		}
	}
...
func TestPricingService_GPT54ProEffortAliasBeatsGenericVariant(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.4": {
	...
	for _, alias := range []string{"gpt-5.4-pro-high", "openai/gpt-5.4-pro-high"} {
		t.Run(alias, func(t *testing.T) {
			catalogPricing := pricingSvc.GetModelPricing(alias)
			require.NotNil(t, catalogPricing)
			require.InDelta(t, 30e-6, catalogPricing.InputCostPerToken, 1e-12)
			require.InDelta(t, 180e-6, catalogPricing.OutputCostPerToken, 1e-12)
			require.Equal(t, 272000, catalogPricing.LongContextInputTokenThreshold)
			...
			cost, err := billing.CalculateCost(alias, UsageTokens{InputTokens: 273000, OutputTokens: 100}, 1)
			require.NoError(t, err)
			require.True(t, cost.LongContextBillingApplied)
			require.InDelta(t, 273000*60e-6, cost.InputCost, 1e-12)
			require.InDelta(t, 100*270e-6, cost.OutputCost, 1e-12)
		})
	}
}

func TestPricingService_GPT55ProEffortAliasBeatsGenericVariant(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.5": {
	...
	for _, alias := range []string{"gpt-5.5-pro-high", "openai/gpt-5.5-pro-high"} {
		t.Run(alias, func(t *testing.T) {
			catalogPricing := pricingSvc.GetModelPricing(alias)
			require.NotNil(t, catalogPricing)
			require.InDelta(t, 30e-6, catalogPricing.InputCostPerToken, 1e-12)
			require.InDelta(t, 180e-6, catalogPricing.OutputCostPerToken, 1e-12)
			require.InDelta(t, 3e-6, catalogPricing.CacheReadInputTokenCost, 1e-12)
			require.Equal(t, 272000, catalogPricing.LongContextInputTokenThreshold)
			...
			cost, err := billing.CalculateCost(alias, UsageTokens{InputTokens: 273000, OutputTokens: 100, CacheReadTokens: 100}, 1)
			require.NoError(t, err)
			require.True(t, cost.LongContextBillingApplied)
			require.InDelta(t, 273000*60e-6, cost.InputCost, 1e-12)
			require.InDelta(t, 100*270e-6, cost.OutputCost, 1e-12)
			require.InDelta(t, 100*6e-6, cost.CacheReadCost, 1e-12)
		})
	}

	staticPricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.5": {InputCostPerToken: 5e-6, OutputCostPerToken: 30e-6},
	}}
	staticPricing := staticPricingSvc.GetModelPricing("gpt-5.5-pro-high")
	require.NotNil(t, staticPricing)
	require.InDelta(t, 30e-6, staticPricing.InputCostPerToken, 1e-12)
	require.InDelta(t, 180e-6, staticPricing.OutputCostPerToken, 1e-12)
	require.InDelta(t, 3e-6, staticPricing.CacheReadInputTokenCost, 1e-12)
	require.Equal(t, 272000, staticPricing.LongContextInputTokenThreshold)

	directFallback, err := NewBillingService(&config.Config{}, nil).GetModelPricing("gpt-5.5-pro-high")
	require.NoError(t, err)
	require.InDelta(t, 30e-6, directFallback.InputPricePerToken, 1e-12)
	require.InDelta(t, 180e-6, directFallback.OutputPricePerToken, 1e-12)
	require.InDelta(t, 3e-6, directFallback.CacheReadPricePerToken, 1e-12)
	require.Equal(t, 272000, directFallback.LongContextInputThreshold)
}
...
func TestGPT56DedicatedFallbacksUseOfficialRates(t *testing.T) {
	tests := []struct {
		...
		{model: "gpt-5.6-sol", input: 4e-6, cached: 0.4e-6, cacheWrite: 5e-6, output: 20e-6, longContext: true},
		{model: "gpt-5.6-terra", input: 2e-6, cached: 0.2e-6, cacheWrite: 2.5e-6, output: 12e-6, longContext: true},
		{model: "gpt-5.6-luna", input: 0.2e-6, cached: 0.02e-6, cacheWrite: 0.25e-6, output: 1.2e-6, longContext: true},
	}
...
func TestGPT56DedicatedFallbacksBillPublishedShortAndLongPrices(t *testing.T) {
	models := []struct {
		name                              string
		input, cached, cacheWrite, output float64
	}{
	...
	sources := map[string]func() *BillingService{
		"PricingService静态回退": func() *BillingService {
			return NewBillingService(&config.Config{}, &PricingService{pricingData: map[string]*LiteLLMModelPricing{
				"gpt-5.1-codex": {InputCostPerToken: 1.25e-6},
			}})
		},
		"BillingService静态回退": func() *BillingService {
	...
	contexts := []struct {
		...
		{name: "短上下文", cacheReadTokens: 72000, inputScale: 1, outputScale: 1},
		{name: "长上下文", cacheReadTokens: 72001, inputScale: 2, outputScale: 1.5, longContextBillingIsExpected: true},
	}
	tiers := []struct {
		name, value string
		scale       float64
		...
		{name: "Fast", value: "priority", scale: 2},
	}

	for source, newBilling := range sources {
		for _, model := range models {
			for _, contextTier := range contexts {
				for _, serviceTier := range tiers {
					t.Run(source+"/"+model.name+"/"+contextTier.name+"/"+serviceTier.name, func(t *testing.T) {
						tokens := UsageTokens{InputTokens: 100000, CacheCreationTokens: 100000, CacheReadTokens: contextTier.cacheReadTokens, OutputTokens: 100}
						cost, err := newBilling().CalculateCostWithServiceTier(model.name, tokens, 1, serviceTier.value)
						require.NoError(t, err)
						require.InDelta(t, float64(tokens.InputTokens)*model.input*serviceTier.scale*contextTier.inputScale, cost.InputCost, 1e-12)
						require.InDelta(t, float64(tokens.CacheCreationTokens)*model.cacheWrite*serviceTier.scale*contextTier.inputScale, cost.CacheCreationCost, 1e-12)
						require.InDelta(t, float64(tokens.CacheReadTokens)*model.cached*serviceTier.scale*contextTier.inputScale, cost.CacheReadCost, 1e-12)
						require.InDelta(t, float64(tokens.OutputTokens)*model.output*serviceTier.scale*contextTier.outputScale, cost.OutputCost, 1e-12)
						require.Equal(t, contextTier.longContextBillingIsExpected, cost.LongContextBillingApplied)
					})
				}
			}
...
func TestGPT56UnknownSuffixDoesNotInheritDedicatedFallback(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.1-codex": {InputCostPerToken: 1.25e-6},
	}}
	...
	for _, model := range []string{"gpt-5.6-sol-preview", "gpt-5.6-terra-preview", "gpt-5.6-luna-preview"} {
		t.Run(model, func(t *testing.T) {
			_, err := withCatalog.GetModelPricing(model)
			require.ErrorIs(t, err, ErrModelPricingUnavailable, "未知产品后缀不得继承目录中的 5.6 价格")

			_, err = withoutCatalog.GetModelPricing(model)
...
func TestWarnDroppedLongContextLadders_WarnsWhenModelEntryDisappears(t *testing.T) {
	logSink, restore := captureStructuredLog(t)
	defer restore()

	old := map[string]*LiteLLMModelPricing{
		"gpt-critical": {LongContextInputTokenThreshold: 272000},
	}
	warnDroppedLongContextLadders(old, map[string]*LiteLLMModelPricing{})

	require.True(t, logSink.ContainsMessageAtLevel("pricing.long_context_ladder_dropped", "warn"))
	require.True(t, logSink.ContainsFieldValue("model_count", "1"))
	require.True(t, logSink.ContainsFieldValue("models", "gpt-critical"))
	require.True(t, logSink.ContainsFieldValue("remediation", "verify catalog/override data"))
}
