...
type codexTicketProxyRoundTripper func(*http.Request) (*http.Response, error)

func (f codexTicketProxyRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req)
}

func TestCodexTicketProxyParserRejectsUnsafeEntries(t *testing.T) {
	...
	_, err = parseCodexTicketProxyBatch([]byte(strings.Repeat(" ", codexTicketProxyBatchMaxBytes+1)), "http")
	require.Error(t, err)
}

func TestCodexTicketProxyParserWhitelistFailureRequiresExactFormat(t *testing.T) {
	for _, tc := range []struct{ body, address string }{
		...
		batch, err := parseCodexTicketProxyBatch([]byte(tc.body), "http")
		require.Nil(t, batch)
		var publicError *codexTicketProxyPublicError
	...
	for _, body := range []string{
		...
		batch, err := parseCodexTicketProxyBatch([]byte(body), "http")
		require.Nil(t, batch)
...
func TestCodexTicketProxyFetchLimitsRedirectsAndRedactsErrors(t *testing.T) {
	...
	input := codexTicketProxyFetchInput{APIURL: "https://supplier.example/secret-path?token=private-token", Protocol: "http"}
	for _, tc := range []struct {
		...
		{"过大", 200, strings.Repeat(" ", codexTicketProxyBatchMaxBytes+1), false, "过大"},
		{"格式错误", 200, "{\"secret\":\"private-token\"}", false, "无效"},
		{"网络错误", 0, "", true, "连接失败"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			calls := 0
			client.Transport = codexTicketProxyRoundTripper(func(req *http.Request) (*http.Response, error) {
				calls++
			...
			batch, err := fetchCodexTicketProxyBatchWithClient(context.Background(), input, client)
			if tc.want == "" {
				require.NoError(t, err)
...
func TestCodexTicketProxyFetchCancellation(t *testing.T) {
	client := newCodexTicketProxyFetchClient()
	...
	_, err := fetchCodexTicketProxyBatchWithClient(ctx, codexTicketProxyFetchInput{APIURL: "https://supplier.example", Protocol: "http"}, client)
	require.Error(t, err)
}

func TestCodexTicketProxyJSONFetchPreservesHTTPURLAndNormalizesEntries(t *testing.T) {
	input := codexTicketProxyFetchInput{
...
func TestCodexTicketProxyJSONRejectsInvalidAndRedactsSupplierErrors(t *testing.T) {
	for _, body := range []string{
	...
	batch, err := parseCodexTicketProxyBatch([]byte(`{"code":400,"success":"","msg":"IP Whitelist Check Failed","error":"IP Whitelist Check Failed","data":[]}`), "http")
	require.Nil(t, batch)
	require.ErrorContains(t, err, "未加入供应商白名单")
	batch, err = parseCodexTicketProxyBatch([]byte(`{"code":200,"success":"success","data":[]}`), "http")
	require.NoError(t, err)
