...
var (
	...
	openAIGPT54ProFallbackPricing = &LiteLLMModelPricing{
		InputCostPerToken:               30e-06,
		OutputCostPerToken:              180e-06,
		CacheCreationInputTokenCost:     30e-06,
...
func (s *PricingService) parsePricingData(body []byte) (map[string]*LiteLLMModelPricing, error) {
	...
	rawData = applyKnownPricingCatalogCorrections(rawData)
...
func warnDroppedLongContextLadders(old, next map[string]*LiteLLMModelPricing) {
	...
	for name, prev := range old {
		...
		cur, ok := next[name]
		if !ok || cur == nil || cur.LongContextInputTokenThreshold <= 0 {
	...
	logger.L().Warn(
		"pricing.long_context_ladder_dropped",
		zap.Int("model_count", total),
		zap.Strings("models", dropped),
		zap.String("remediation", "verify catalog/override data if unintended"),
	)
}
...
func (s *PricingService) matchOpenAIModel(model string) *LiteLLMModelPricing {
	...
	// 产品 SKU 必须先于 generateOpenAIModelVariants：后者会把
	// gpt-5.4-pro-high 等别名缩成 gpt-5.4，提前命中普通 5.4 目录价。
	normalized := canonicalizeOpenAIModelSpelling(model)
	if policy, ok := resolveOpenAIFastModelPolicy(normalized); ok {
		var fallback *LiteLLMModelPricing
		switch policy.CanonicalSKU {
		case "gpt-5.6-sol":
		...
		case "gpt-5.4-mini":
			fallback = openAIGPT54MiniFallbackPricing
		}
		if fallback != nil {
			if pricing, exists := s.pricingData[policy.CanonicalSKU]; exists {
				return pricing
			}
			logger.With(zap.String("component", "service.pricing")).
				Info(fmt.Sprintf("[Pricing] OpenAI fallback matched %s -> %s", model, policy.CanonicalSKU+"(static)"))
			return fallback
		}
	}
	for _, dedicated := range []struct {
		sku      string
		fallback *LiteLLMModelPricing
	}{
		{sku: "gpt-5.5-pro", fallback: openAIGPT55ProFallbackPricing},
		{sku: "gpt-5.4-pro", fallback: openAIGPT54ProFallbackPricing},
		{sku: "gpt-5.4-nano", fallback: openAIGPT54NanoFallbackPricing},
	} {
		if !openAIModelMatchesFastSKU(normalized, dedicated.sku) {
			continue
		}
		if pricing, exists := s.pricingData[dedicated.sku]; exists {
			return pricing
		}
		logger.With(zap.String("component", "service.pricing")).
			Info(fmt.Sprintf("[Pricing] OpenAI fallback matched %s -> %s", model, dedicated.sku+"(static)"))
