...
func TestUserCanBindGroup(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:    "a public group blacklist takes priority over the restricted allow list",
			user:    User{RestrictPublicGroups: true, AllowedGroups: []int64{publicGroupA}, BlockedGroups: []int64{publicGroupA}},
			groupID: publicGroupA,
			want:    false,
		},
		{
