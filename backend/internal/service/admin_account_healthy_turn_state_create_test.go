...
func TestAdminCreateOAuthHealthyTurnStateSwitchFromJSON(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "未提供策略默认门票", extra: `{}`, ticket: true},
		{name: "字符串开关不能误启用", extra: `{"openai_healthy_turn_state_replace":"true"}`, invalid: true},
		{name: "高级商务新账号默认门票", extra: `{}`, plan: "self_serve_business_prolite", ticket: true},
		{name: "高级商务未提供额外字段默认门票", extra: `null`, plan: "self_serve_business_prolite", ticket: true},
		{name: "默认门票不依赖档位格式", extra: `{}`, plan: " Self-Serve_Business Prolite ", ticket: true},
		...
		{name: "标准商务默认门票", extra: `{}`, plan: "team", ticket: true},
		{name: "个人轻量专业版默认门票", extra: `{}`, plan: "prolite", ticket: true},
		{name: "未知档位默认门票", extra: `{}`, plan: "Business Premium", ticket: true},
		{name: "非字符串档位也不影响默认门票", extra: `{}`, plan: 42, ticket: true},
		...
		{name: "初始化令牌默认门票", extra: `{}`, accountType: AccountTypeSetupToken, ticket: true},
		{name: "显式异常补试优先", extra: `{"openai_turn_state_mode":"healthy_retry"}`, enabled: true},
		{name: "显式首发优先", extra: `{"openai_turn_state_mode":"healthy_preflight"}`, enabled: true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.ticket {
				require.Equal(t, false, persisted.Extra[openAIHealthyTurnStateReplaceKey], "默认门票必须关闭旧健康池维护")
			}
...
func TestCRSSyncTurnStateDefaultsOnlyNewAccounts(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "新账号默认门票", mode: OpenAITurnStateCodexTicket},
