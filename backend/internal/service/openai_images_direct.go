...
func buildOpenAIImagesOAuthPayload(parsed *OpenAIImagesRequest, model string) ([]byte, string, error) {
	return buildOpenAIImagesOAuthPayloadWithContext(context.Background(), parsed, model)
}

func buildOpenAIImagesOAuthPayloadWithContext(ctx context.Context, parsed *OpenAIImagesRequest, model string) ([]byte, string, error) {
	if parsed == nil {
		return nil, "", fmt.Errorf("parsed images request is required")
	}
	if !usesCodexDirectImages(model) {
		body, err := buildOpenAIImagesResponsesRequest(ctx, parsed, model)
...
func (s *OpenAIGatewayService) handleCodexDirectImagesNonStreamingResponse(resp *http.Response, c *gin.Context, opts openAIImagesNonStreamingResponseOptions) (OpenAIUsage, int, []string, error) {
	parsed := opts.parsed
	...
	// 原始用量在增强前读取；响应处理沿用统一的增强和图片存储契约。
	buffered := *resp
	buffered.Body = io.NopCloser(bytes.NewReader(body))
	_, _, _, err = s.handleOpenAIImagesNonStreamingResponse(&buffered, c, opts)
	if err != nil {
		return OpenAIUsage{}, 0, nil, err
	}
