...
// ValidateOpenAIImagesNativeModel 校验默认 Images 路径支持的 OpenAI 或兼容供应商图片模型。
...
func (s *OpenAIGatewayService) buildOpenAIImagesAPIKeyForwardBody(
	...
	// Gemini 兼容图片接口保留供应商 JSON 字段与引用 URL；原生 OpenAI
	// 图片编辑仍转换为 multipart，保持既有上传兼容行为。
	if !isGeminiCompatibleImageModel(model) && parsed.IsEdits() && !parsed.Multipart && (len(parsed.InputImageURLs) > 0 || len(parsed.Uploads) > 0) {
...
func (s *OpenAIGatewayService) handleOpenAIImagesNonStreamingResponse(resp *http.Response, c *gin.Context, opts openAIImagesNonStreamingResponseOptions) (OpenAIUsage, int, []string, error) {
	...
	body = s.backfillOpenAIImagesB64JSON(opts.ctx, opts.account, opts.parsed, body)
...
func (s *OpenAIGatewayService) rewriteOpenAIImagesStreamingJSONBody(
	...
	return s.localizeOpenAIImagesJSONResponse(ctx, c, body, opts.nonStreamingOptions())
}
...
func (s *OpenAIGatewayService) rewriteOpenAIImagesStreamingCompletedPayload(
	...
	return s.localizeOpenAIImagesStreamingPayload(ctx, c, payload, opts)
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesStreamingResponse(
	...
	processSSEData := func(dataBytes []byte) {
		...
		if strings.HasSuffix(eventType, ".completed") {
			...
			dataBytes, rewriteErr = s.rewriteOpenAIImagesStreamingCompletedPayload(opts.ctx, c, dataBytes, opts)
			if rewriteErr != nil {
				streamErr = rewriteErr
				return
			}
		}
