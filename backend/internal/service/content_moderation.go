	...
	if cfg.Mode == ContentModerationModePreBlock {
		if cfg.KeywordBlockingMode != ContentModerationKeywordModeAPIOnly && len(cfg.BlockedKeywords) > 0 {
			...
			if keyword, hit := runtimeSnapshot.matchBlockedKeyword(keywordText); hit {
				...
				log := s.buildLog(input, cfg, ContentModerationActionKeywordBlock, true, contentModerationKeywordCategory, 1.0, scores, extractContentModerationInput(input.Protocol, input.Body, false), nil, nil, "")
