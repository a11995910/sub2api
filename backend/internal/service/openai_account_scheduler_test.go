...
func TestOpenAIGatewayService_SelectAccountForTokenCount_AutoFallbackUsesTargetMappingWithoutSlot(t *testing.T) {
	targetID := int64(10117)
	source := autoFallbackTestGroup(10116, 0.12, &targetID)
	source.MessagesDispatchModelConfig.SonnetMappedModel = "source-sonnet"
	target := autoFallbackTestGroup(targetID, 0.18, nil)
	target.MessagesDispatchModelConfig.SonnetMappedModel = "target-sonnet"
	...
	accountRepo := &autoFallbackAccountRepoStub{accounts: []Account{
		{
			...
			AccountGroups:    []AccountGroup{{GroupID: source.ID}},
			Credentials: map[string]any{
				"openai_capabilities": []any{"chat_completions"},
				"model_mapping":       map[string]any{"source-sonnet": "source-sonnet"},
			},
		},
		{
			ID:            36512,
			...
			Schedulable:   true,
			Concurrency:   1,
			AccountGroups: []AccountGroup{{GroupID: target.ID}},
			Credentials: map[string]any{
				"openai_capabilities": []any{"chat_completions"},
				"model_mapping":       map[string]any{"target-sonnet": "target-sonnet"},
			},
		},
	}}
	acquiredIDs := make([]int64, 0)
	...
	require.Equal(t, "target-sonnet", apiKey.Group.ResolveMessagesDispatchModel("claude-sonnet-4-5"))
	require.Empty(t, acquiredIDs, "自动承接的 token 计数不得申请生成并发槽")
}
