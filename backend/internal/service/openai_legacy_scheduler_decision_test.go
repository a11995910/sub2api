...
func TestLegacySchedulerDecision_StickySessionLayer(t *testing.T) {
	...
	for _, scenario := range []struct {
		...
		loadBatch          bool
		samePriority       bool
		higherPriorityLoad int
		wantID             int64
		wantSticky         bool
	}{
		{name: "批量调度高优先级有空槽", loadBatch: true, wantID: 38101},
		{name: "批量调度高优先级已满", loadBatch: true, higherPriorityLoad: 100, wantID: 38102, wantSticky: true},
		{name: "批量调度同优先级保持粘性", loadBatch: true, samePriority: true, wantID: 38102, wantSticky: true},
		{name: "优先级LRU保持粘性", wantID: 38102, wantSticky: true},
	} {
		t.Run(scenario.name, func(t *testing.T) {
			accounts := newLegacySchedulerDecisionTestAccounts(groupID, true)
			if scenario.samePriority {
				accounts[0].Priority = accounts[1].Priority
			}
			svc := newLegacySchedulerDecisionTestService(accounts, scenario.loadBatch, schedulerTestConcurrencyCache{
				loadMap: map[int64]*AccountLoadInfo{38101: {AccountID: 38101, LoadRate: scenario.higherPriorityLoad}},
			})
			...
			if scenario.wantSticky {
				wantLayer = openAIAccountScheduleLayerSessionSticky
			}
			require.Equal(t, wantLayer, decision.Layer)
			require.Equal(t, scenario.wantSticky, decision.StickySessionHit)
			require.False(t, decision.StickyPreviousHit)
			require.Equal(t, scenario.wantID, decision.SelectedAccountID)
			require.Equal(t, AccountTypeAPIKey, decision.SelectedAccountType)

			// 无会话绑定时重新使用空闲账号池，仍按优先级选择并报告负载均衡层。
			svc = newLegacySchedulerDecisionTestService(newLegacySchedulerDecisionTestAccounts(groupID, true), scenario.loadBatch, schedulerTestConcurrencyCache{})
			selection, decision, err = svc.SelectAccountWithSchedulerForCapability(
