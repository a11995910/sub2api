...
const (
	...
	// Antigravity OAuth 客户端 ID 不在源码中内置，运行环境需通过环境变量注入。
	ClientID = ""
...
var (
	// defaultClientID/defaultClientSecret 可通过环境变量配置；源码不内置第三方 OAuth 凭证。
	defaultClientID     = ClientID
	defaultClientSecret string
)

func init() {
	// 从环境变量读取 client_id，未设置则保持空值并在 OAuth 流程入口返回明确错误
