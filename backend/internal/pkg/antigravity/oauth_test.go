...
func setTestAntigravityClientID(t *testing.T) {
	t.Helper()
	old := defaultClientID
...
func TestConstants_值正确(t *testing.T) {
	...
	if ClientID != "" {
		t.Errorf("ClientID 不应在源码中内置: got %s", ClientID)
	}
	...
	secret, err := getClientSecret()
	if err != nil {
		t.Fatalf("getClientSecret 应返回配置值，但报错: %v", err)
	}
	if secret != "test-secret" {
		t.Errorf("client_secret 不匹配: got %s", secret)
	}
