...
const testGeminiCLIBuiltinClientID = "test-gemini-cli-client-id"

func setTestGeminiCLIBuiltinOAuth(t *testing.T, secret string) {
	t.Helper()
	t.Setenv(GeminiCLIOAuthClientIDEnv, testGeminiCLIBuiltinClientID)
...
func TestBuildAuthorizationURL(t *testing.T) {
	setTestGeminiCLIBuiltinOAuth(t, "test-secret")
	...
	checks := []string{
		"response_type=code",
		"client_id=" + testGeminiCLIBuiltinClientID,
		"redirect_uri=",
		"state=test-state",
...
func TestBuildAuthorizationURL_MissingBuiltinClientConfig(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientIDEnv, testGeminiCLIBuiltinClientID)
...
func TestEffectiveOAuthConfig_GoogleOne(t *testing.T) {
	// 内置的 Gemini CLI client ID/secret 不嵌入在此仓库中。
	// 测试通过环境变量设置假值来模拟运维配置。
	setTestGeminiCLIBuiltinOAuth(t, "test-built-in-secret")
	...
	tests := []struct {
	...
	}{
		{
			...
			wantClientID: testGeminiCLIBuiltinClientID,
...
func TestEffectiveOAuthConfig_MissingBuiltinEnv(t *testing.T) {
	...
	if err == nil {
		t.Fatal("不设置内置客户端 secret 时应返回错误")
	}
}
