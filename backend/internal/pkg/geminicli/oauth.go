...
// BuiltinOAuthClientID 返回运行环境配置的 Gemini CLI 内置 OAuth client_id。
func BuiltinOAuthClientID() string {
	if id := strings.TrimSpace(GeminiCLIOAuthClientID); id != "" {
		return id
	}
	return strings.TrimSpace(os.Getenv(GeminiCLIOAuthClientIDEnv))
}

// IsBuiltinOAuthClientID 判断给定 client_id 是否是当前运行环境配置的内置客户端。
func IsBuiltinOAuthClientID(clientID string) bool {
	builtinID := BuiltinOAuthClientID()
	return builtinID != "" && strings.TrimSpace(clientID) == builtinID
}
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// SECURITY: This repo does not embed the built-in client ID/secret; they must be provided via env.
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		if clientID == "" || secret == "" {
			return OAuthConfig{}, infraerrors.Newf(http.StatusBadRequest, "GEMINI_CLI_OAUTH_CLIENT_NOT_CONFIGURED", "built-in Gemini CLI OAuth client is not configured; set %s and %s or provide a custom OAuth client", GeminiCLIOAuthClientIDEnv, GeminiCLIOAuthClientSecretEnv)
		}
