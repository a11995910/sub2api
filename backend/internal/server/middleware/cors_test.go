...
func TestCORS_AllowedOrigin_HasAllowHeaders(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Contains(t, w.Header().Get("Access-Control-Allow-Headers"), ModelTestRequestHeader)
			assert.Contains(t, w.Header().Get("Access-Control-Allow-Headers"), ModelTestAuthorizationHeader)
