...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, deferredBodyAdmission, gin.HandlerFunc(apiKeyAuth), bodyAdmission, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)`))
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, deferredBodyAdmission, gin.HandlerFunc(apiKeyAuth), bodyAdmission, groupModelAllowlist, compositeTarget, requireGroupAnthropic)`))
