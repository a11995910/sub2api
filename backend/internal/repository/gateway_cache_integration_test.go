...
func (s *GatewayCacheSuite) TestAdjustCacheHitToTargetCumulative() {
	...
	adjustment, err := tracker.AdjustCacheHitToTarget(s.ctx, "integration-1", 1001, 2001, 9000, 50, 86400, 1, 100, 80)
	require.NoError(s.T(), err)
	require.Zero(s.T(), adjustment.ShiftedTokens)

	adjustment, err = tracker.AdjustCacheHitToTarget(s.ctx, "integration-2", 1001, 2001, 9000, 50, 86400, 1, 100, 100)
	require.NoError(s.T(), err)
	require.Zero(s.T(), adjustment.ShiftedTokens)

	adjustment, err = tracker.AdjustCacheHitToTarget(s.ctx, "integration-3", 1001, 2001, 9000, 50, 86400, 1, 100, 100)
	require.NoError(s.T(), err)
