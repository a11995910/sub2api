...
func TestAffiliateRepository_ListAffiliateRebateRecords_IncludesNonOrderAccruals(t *testing.T) {
	...
	applied, err := repo.AccrueQuota(txCtx, inviter.ID, invitee.ID, 2, 0, nil, nil)
	require.NoError(t, err)
...
// 兑换码返利保留完整来源与面值，关联不到支付订单时仍返回空订单标识。
func TestAffiliateRepository_ListAffiliateRebateRecords_PreservesRedeemCodeSource(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	txCtx := dbent.NewTxContext(ctx, tx)
	client := tx.Client()
	...
	inviter := mustCreateAffiliateWithdrawUser(t, client, "redeem-inviter")
	invitee := mustCreateAffiliateWithdrawUser(t, client, "redeem-invitee")
	mustSeedAffiliateQuota(t, txCtx, client, inviter.ID, 0, 0, 0)
	code := fmt.Sprintf("返利-%d", time.Now().UnixNano())
	codeID := int64(querySingleInt(t, txCtx, client,
		"INSERT INTO redeem_codes (code, type, value, status, used_by) VALUES ($1, 'balance', 20, 'used', $2) RETURNING id",
		code, invitee.ID))
	applied, err := repo.AccrueQuota(txCtx, inviter.ID, invitee.ID, 2, 0, nil, &codeID)
	require.NoError(t, err)
	require.True(t, applied)

	records, total, err := repo.ListAffiliateRebateRecords(txCtx, service.AffiliateRecordFilter{
		Search: code, Page: 1, PageSize: 20, SortDesc: true,
	})
	require.NoError(t, err)
	require.EqualValues(t, 1, total)
	require.Len(t, records, 1)
	record := records[0]
	require.Positive(t, record.LedgerID)
	require.Equal(t, "redeem_code", record.SourceType)
	require.Equal(t, codeID, record.SourceID)
	require.Equal(t, codeID, record.RedeemCodeID)
	require.Equal(t, code, record.RedeemCode)
	require.Nil(t, record.OrderID)
	require.Empty(t, record.OutTradeNo)
	require.NotNil(t, record.OrderAmount)
	require.NotNil(t, record.PayAmount)
	require.InDelta(t, 20, *record.OrderAmount, 1e-9)
	require.InDelta(t, 20, *record.PayAmount, 1e-9)
	require.Equal(t, "redeem_code", record.PaymentType)
	require.Equal(t, "used", record.OrderStatus)
	require.InDelta(t, 2, record.RebateAmount, 1e-9)
}
