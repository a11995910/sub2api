...
func TestGatewayCacheAdjustCacheHitToTarget_CumulativeAndIsolated(t *testing.T) {
	...
	adjustment, err := cache.AdjustCacheHitToTarget(ctx, 1, 2, 9000, 50, 101, 100, 80)
	require.NoError(t, err)
	require.Zero(t, adjustment.ShiftedTokens)

	// 单次请求是 100% 命中，但与上一次累计后恰好为 90%，不应误降。
	adjustment, err = cache.AdjustCacheHitToTarget(ctx, 1, 2, 9000, 50, 101, 100, 100)
	require.NoError(t, err)
	require.Zero(t, adjustment.ShiftedTokens)
	...
	// 不同用户使用独立累计状态。
	adjustment, err = cache.AdjustCacheHitToTarget(ctx, 3, 2, 9000, 50, 101, 100, 94)
	require.NoError(t, err)
	require.Equal(t, 4, adjustment.ShiftedTokens)

	ttl, err := cache.rdb.TTL(ctx, "cache_hit_target:v2:1:2:9000:50:101").Result()
	require.NoError(t, err)
...
func TestGatewayCacheAdjustCacheHitToTarget_ConcurrentAtomicity(t *testing.T) {
	...
	for range requests {
		...
		go func() {
			defer wg.Done()
			adjustment, err := cache.AdjustCacheHitToTarget(ctx, 11, 22, 9000, 50, 202, 100, 94)
...
func TestGatewayCacheAdjustCacheHitToTarget_StateVersionIsolation(t *testing.T) {
	...
	first, err := cache.AdjustCacheHitToTarget(ctx, 21, 22, 9000, 50, 301, 100, 80)
	require.NoError(t, err)
	require.Zero(t, first.ShiftedTokens)
	second, err := cache.AdjustCacheHitToTarget(ctx, 21, 22, 9000, 50, 301, 100, 100)
	require.NoError(t, err)
	require.Zero(t, second.ShiftedTokens)

	// 分组保存后状态代次变化，新代次不继承旧累计。
	reset, err := cache.AdjustCacheHitToTarget(ctx, 21, 22, 9000, 50, 302, 100, 100)
	require.NoError(t, err)
...
func TestGatewayCacheAdjustCacheHitToTarget_DoesNotShiftHistoricalToleranceBuffer(t *testing.T) {
	...
	atUpperBound, err := cache.AdjustCacheHitToTarget(ctx, 31, 32, 9000, 50, 401, 10000, 9050)
	require.NoError(t, err)
	require.Zero(t, atUpperBound.ShiftedTokens)

	// 触发时回到目标需要 51 token，但本次只有 1 个缓存读取 token，因此最多只能划 1 个。
	crossed, err := cache.AdjustCacheHitToTarget(ctx, 31, 32, 9000, 50, 401, 1, 1)
	require.NoError(t, err)
