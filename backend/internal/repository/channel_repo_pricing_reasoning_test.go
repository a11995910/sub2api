...
func reasoningPricingRow(multipliers any) *sqlmock.Rows {
	return sqlmock.NewRows(channelModelPricingTimePricingColumns).AddRow(
		int64(11), int64(7), "openai", `["custom-model"]`, service.BillingModeToken, service.PriceCurrencyUSD,
	...
	for _, tt := range tests {
		for _, operation := range []string{"create", "update", "replace"} {
			t.Run(tt.name+"/"+operation, func(t *testing.T) {
				...
				if operation == "update" {
					mock.ExpectExec(`(?s)UPDATE channel_model_pricing.*reasoning_effort_multipliers = \$11.*WHERE id = \$17`).
						WithArgs([]byte(`["custom-model"]`), service.BillingModeToken, service.PriceCurrencyUSD,
					...
					mock.ExpectQuery(`INSERT INTO channel_model_pricing .*reasoning_effort_multipliers`).
						WithArgs(int64(7), "openai", []byte(`["custom-model"]`), service.BillingModeToken, service.PriceCurrencyUSD,
...
func TestAccountStatsReasoningEffortMultipliersRoundTrip(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO channel_account_stats_model_pricing .*reasoning_effort_multipliers`).
		WithArgs(int64(9), "anthropic", []byte(`["custom-model"]`), service.BillingModeToken, service.PriceCurrencyUSD,
	...
	columns := []string{
		...
		"image_input_price", "image_output_price", "per_request_price", "created_at", "updated_at",
	}
	mock.ExpectQuery(`(?s)SELECT .*reasoning_effort_multipliers.*FROM channel_account_stats_model_pricing`).
		...
		int64(11), int64(9), "anthropic", `["custom-model"]`, service.BillingModeToken, service.PriceCurrencyUSD,
