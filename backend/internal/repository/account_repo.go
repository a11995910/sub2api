...
func lockAndMergeAccountProbeExtra(
	...
	if len(currentExtraJSON) > 0 {
		if err := json.Unmarshal(currentExtraJSON, &currentExtra); err != nil {
			logger.LegacyPrintf("repository.account", "账号额外字段解析失败，跳过门票保留: id=%d", account.ID)
			currentExtra = nil
		}
	}
...
func (r *accountRepository) ListIDsWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string) ([]int64, *pagination.PaginationResult, error) {
	...
	// Clone before Count so interceptor-appended predicates don't accumulate on
	// the shared builder and pollute the subsequent list query.
	total, err := q.Clone().Count(ctx)
	if err != nil {
		return nil, nil, err
	}
...
func (r *accountRepository) loadProxies(ctx context.Context, proxyIDs []int64) (map[int64]*service.Proxy, error) {
	...
	proxyIDs = uniquePositiveAccountInt64s(proxyIDs)
	if len(proxyIDs) == 0 {
		return proxyMap, nil
	}
...
func (r *accountRepository) loadAccountGroups(ctx context.Context, accountIDs []int64) (map[int64][]*service.Group, map[int64][]int64, map[int64][]service.AccountGroup, error) {
	...
	accountIDs = uniquePositiveAccountInt64s(accountIDs)
	if len(accountIDs) == 0 {
...
func (r *accountRepository) loadGroups(ctx context.Context, groupIDs []int64) (map[int64]*service.Group, error) {
	...
	groupIDs = uniquePositiveAccountInt64s(groupIDs)
	if len(groupIDs) == 0 {
		return groupMap, nil
	}
...
func uniquePositiveAccountInt64s(ids []int64) []int64 {
	if len(ids) == 0 {
		return nil
	}
