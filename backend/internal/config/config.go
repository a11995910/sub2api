...
const DefaultCSPPolicy = "default-src 'self'; worker-src 'self' blob:; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://*.alicdn.com https://static.cloudflareinsights.com https://turing.captcha.qcloud.com https://turing.captcha.gtimg.com https://ca.turing.captcha.qcloud.com https://global.turing.captcha.gtimg.com https://www.tycaptcha.com https://cloudcache.tencentcs.com https://*.stripe.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; style-src 'self' 'unsafe-inline' https://*.captcha.gtimg.com https://fonts.googleapis.com https://*.alicdn.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; img-src 'self' data: blob: https:; media-src 'self' blob: https://vidgen.x.ai; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https://turing.captcha.qcloud.com https://www.tycaptcha.com https://rce.tencentrio.com https:; frame-src 'self' https://challenges.cloudflare.com https://turing.captcha.qcloud.com https://ca.turing.captcha.qcloud.com https://www.tycaptcha.com https://*.stripe.com https://checkout.airwallex.com https://checkout-demo.airwallex.com https://pay.ldxp.cn; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
type GatewayConfig struct {
	...
	// OpenAICodexTicket 控制独立 292/332 门票实验，默认关闭。
...
type TLSProfileConfig struct {
	...
	// ALPNProtocols: ALPN协议列表。只支持 ["http/1.1"]（留空即用该默认值）。
	// 指纹 transport 恒为 HTTP/1.1，填入 "h2" 会让服务端选中一个客户端不会说的协议，
	// 导致该账号所有请求被上游重置；真实 Claude Code 也是 http/1.1（JA4 末尾的 h1）。
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	imageStorageEnvBindings := []struct {
		...
		{configKey: "image_storage.enabled", envKey: "IMAGE_STORAGE_ENABLED"},
		{configKey: "image_storage.endpoint", envKey: "IMAGE_STORAGE_ENDPOINT"},
		{configKey: "image_storage.region", envKey: "IMAGE_STORAGE_REGION"},
		{configKey: "image_storage.bucket", envKey: "IMAGE_STORAGE_BUCKET"},
		{configKey: "image_storage.access_key_id", envKey: "IMAGE_STORAGE_ACCESS_KEY_ID"},
		{configKey: "image_storage.secret_access_key", envKey: "IMAGE_STORAGE_SECRET_ACCESS_KEY"},
		{configKey: "image_storage.prefix", envKey: "IMAGE_STORAGE_PREFIX"},
		{configKey: "image_storage.force_path_style", envKey: "IMAGE_STORAGE_FORCE_PATH_STYLE"},
		{configKey: "image_storage.public_base_url", envKey: "IMAGE_STORAGE_PUBLIC_BASE_URL"},
		{configKey: "image_storage.presign_expiry_hours", envKey: "IMAGE_STORAGE_PRESIGN_EXPIRY_HOURS"},
		{configKey: "image_storage.max_download_bytes", envKey: "IMAGE_STORAGE_MAX_DOWNLOAD_BYTES"},
	}
	for _, binding := range imageStorageEnvBindings {
		if err := viper.BindEnv(binding.configKey, binding.envKey); err != nil {
			return nil, fmt.Errorf("bind %s: %w", binding.envKey, err)
		}
	}
...
func setDefaults() {
	...
	// 定价：从 model-price-repo main 分支同步模型定价和上下文窗口数据
...
// 标准 target_length（292 或 332）同时接受两种长度；其他值精确匹配。
