...
func (h *AvailableChannelHandler) loadPersistentAccountsByGroup(
	...
	for _, group := range groups {
		...
		if group.Platform == service.PlatformComposite {
			platforms = append([]string(nil), service.AllowedQuotaPlatforms...)
		}
...
func buildPlatformSections(
	...
	for _, platform := range platforms {
		...
		if len(section.Groups) == 0 || (platform != service.PlatformComposite && len(section.SupportedModels) == 0) {
			continue
		}
...
func toUserPricing(p *service.ChannelModelPricing) *userSupportedModelPricing {
	...
	return &userSupportedModelPricing{
		...
		PriceCurrency:    string(p.PriceCurrency.OrDefault()),
