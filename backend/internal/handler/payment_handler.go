...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	for _, p := range plans {
		...
		result = append(result, planWithPlatform{
			ID: int64(p.ID), GroupID: p.GroupID, GroupPlatform: gi.Platform, GroupName: gi.Name,
			...
			PromoDiscountEnabled: gi.PromoDiscountEnabled, PromoDiscountStart: gi.PromoDiscountStart,
			PromoDiscountEnd: gi.PromoDiscountEnd, PromoDiscountRate: gi.PromoDiscountRate,
			PromoActive:   gi.PromoActive,
...
type purchaseSubscriptionWithBalanceResponse struct {
	...
	RedeemCode   *dto.RedeemCode       `json:"redeem_code"`
	Subscription *dto.UserSubscription `json:"subscription,omitempty"`
}

// PurchasePlanWithBalance 使用灵石兑换订阅套餐。
...
func (h *PaymentHandler) PurchasePlanWithBalance(c *gin.Context) {
	...
	result, err := h.paymentService.PurchaseSubscriptionWithBalance(c.Request.Context(), subject.UserID, planID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	response.Success(c, purchaseSubscriptionWithBalanceResponse{
		PlanID:       result.PlanID,
		Price:        result.Price,
		NewBalance:   result.NewBalance,
		RedeemCode:   dto.RedeemCodeFromService(result.RedeemCode),
