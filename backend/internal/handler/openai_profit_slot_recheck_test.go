...
func TestAcquireResponsesAccountSlotProfitRecheck(t *testing.T) {
	...
	t.Run("veto releases slot and requests reschedule without writing response", func(t *testing.T) {
		...
		release, result, failover := h.acquireResponsesAccountSlotWithFailover(c, &groupID, "", newSelection(profitSlotTestAccount(1, 0.8)), false, &streamStarted, zap.NewNop())
	...
	t.Run("qualifying account acquires normally", func(t *testing.T) {
		...
		release, result, failover := h.acquireResponsesAccountSlotWithFailover(c, &groupID, "", newSelection(profitSlotTestAccount(2, 0.3)), false, &streamStarted, zap.NewNop())
		require.Nil(t, failover)
	...
	t.Run("image intent suppression keeps official behavior", func(t *testing.T) {
		...
		release, result, failover := h.acquireResponsesAccountSlotWithFailover(c, &groupID, "", newSelection(profitSlotTestAccount(3, 0.8)), false, &streamStarted, zap.NewNop())
		require.Nil(t, failover)
