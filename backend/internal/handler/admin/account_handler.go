...
func (h *AccountHandler) List(c *gin.Context) {
	...
	if includeSchedulerScore && pageHasOpenAIAccounts {
		schedulerFilterPool := h.listAccountSchedulerScoreFilterPool(c.Request.Context(), query.platform, query.accountType, query.status, query.search, query.groupID, query.privacyMode)
	...
	if query.lite {
		...
		etag := buildAccountsListETag(compact, total, query.page, query.pageSize, query.platform, query.accountType, query.status, query.search, true)
		if etag != "" {
			c.Header("ETag", etag)
		...
		response.Paginated(c, compact, total, query.page, query.pageSize)
		return
	}

	etag := buildAccountsListETag(result, total, query.page, query.pageSize, query.platform, query.accountType, query.status, query.search, false)
	if etag != "" {
		c.Header("ETag", etag)
		c.Header("Vary", "If-None-Match")
		if ifNoneMatchMatched(c.GetHeader("If-None-Match"), etag) {
