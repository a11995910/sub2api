...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		OAuthPoolVisible:                req.OAuthPoolVisible,
...
func (h *GroupHandler) GetStats(c *gin.Context) {
	...
	keys, total, err := h.adminService.GetGroupAPIKeys(c.Request.Context(), groupID, 1, 1)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	response.Success(c, gin.H{"id": group.ID, "name": group.Name, "total_api_keys": total, "active_api_keys": active})
}
