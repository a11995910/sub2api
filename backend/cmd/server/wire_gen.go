...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	affiliateService := service.ProvideAffiliateService(affiliateRepository, settingService, groupRepository, userRepository, userSubscriptionRepository, apiKeyAuthCacheInvalidator, billingCacheService)
	...
	healthyTurnStateRepository := repository.NewHealthyTurnStateRepository(db, secretEncryptor)
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, healthyTurnStateRepository)
	...
	oAuthAccountPoolRepository := repository.NewOAuthAccountPoolRepository(client)
	...
	oAuthAccountPoolService := service.NewOAuthAccountPoolService(apiKeyService, oAuthAccountPoolRepository, accountUsageService)
	oAuthAccountPoolHandler := handler.NewOAuthAccountPoolHandler(oAuthAccountPoolService)
	...
	checkinRepository := repository.NewCheckinRepository(client)
	checkinService := service.NewCheckinService(checkinRepository, userRepository, settingService, client, apiKeyAuthCacheInvalidator, billingCacheService)
	checkinHandler := handler.NewCheckinHandler(checkinService)
	lotteryRepository := repository.NewLotteryRepository(db)
	lotteryService := service.NewLotteryService(lotteryRepository, apiKeyAuthCacheInvalidator, billingCacheService)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, settingService, pluginManager, settingRepository, secretEncryptor)
	...
	adminLotteryHandler := admin.NewLotteryHandler(lotteryService)
	...
	upstreamRateMonitorRepository := repository.NewUpstreamRateMonitorRepository(db)
	upstreamRateMonitorService := service.NewUpstreamRateMonitorService(upstreamRateMonitorRepository, secretEncryptor)
	upstreamRateMonitorHandler := admin.NewUpstreamRateMonitorHandler(upstreamRateMonitorService)
	...
	videoTaskReviewRepository := repository.NewVideoTaskReviewRepository(db)
	videoTaskBillingRepository := repository.NewVideoTaskBillingRepository(db)
	videoTaskUsageService := service.ProvideVideoTaskUsageService(openAIGatewayService, apiKeyRepository, userRepository, accountRepository, apiKeyService)
	videoTaskBillingService := service.ProvideVideoTaskBillingService(videoTaskBillingRepository, openAIGatewayService, videoTaskUsageService, billingCacheService)
	videoTaskReviewService := service.ProvideVideoTaskReviewService(videoTaskReviewRepository, videoTaskBillingService, openAIGatewayService, accountRepository)
	videoTaskReviewHandler := admin.NewVideoTaskReviewHandler(videoTaskReviewService)
	...
	generatedImageStore := service.ProvideGeneratedImageStore(configConfig)
	videoTestTaskStore := repository.NewVideoTestTaskRepository(db)
	videoTestTaskService := service.NewVideoTestTaskService(videoTestTaskStore)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, generatedImageStore, grokQuotaService, videoTestTaskService, videoTaskBillingService, configConfig, coordinator)
	...
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService, accountRepository)
	...
	imageTaskRepository := repository.NewImageTaskStore(redisClient)
	imageTaskService := service.ProvideImageTaskService(imageTaskRepository, imageStorageSettingService)
	asyncImageHandler := handler.ProvideAsyncImageHandler(imageTaskService, openAIGatewayHandler, subscriptionService)
	...
	generatedImageHandler := handler.NewGeneratedImageHandler(generatedImageStore)
	videoTestTaskGateway := handler.ProvideVideoTestTaskGateway(openAIGatewayService)
	videoTestTaskContentStore := service.ProvideVideoTestTaskContentStore(configConfig)
	videoTestTaskHandler := handler.NewVideoTestTaskHandler(videoTestTaskService, videoTestTaskGateway, videoTestTaskContentStore)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	generatedImageCleanupService := service.ProvideGeneratedImageCleanupService(generatedImageStore)
	videoTestTaskCleanupService := service.ProvideVideoTestTaskCleanupService(videoTestTaskStore, videoTestTaskContentStore)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, oAuthAccountPoolHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, checkinHandler, lotteryHandler, creativeDrawingHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, generatedImageHandler, videoTestTaskHandler, idempotencyCoordinator, idempotencyCleanupService, generatedImageCleanupService, videoTestTaskCleanupService, openAIQuotaAutoResetService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService, auditLogService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig)
	modelTestAuthMiddleware := middleware.NewModelTestAuthMiddleware(authService, userService, settingService, auditLogService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, modelTestAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient)
	...
	imageTaskWorkerRuntime := service.ProvideImageTaskWorkerRuntime(imageTaskService, asyncImageHandler)
	videoTaskReconciliationService := service.ProvideVideoTaskReconciliationService(videoTaskBillingService, openAIGatewayService, accountRepository)
