...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("cache_hit_half_life_days").
			SchemaType(map[string]string{dialect.Postgres: "decimal(8,2)"}).
			Default(1.0).
			Min(0.01).
			Max(365).
			Comment("缓存命中率累计状态的时间半衰期（天）；历史权重按时间指数衰减"),
...
func (Group) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("blocked_users", User.Type).
			Ref("blocked_groups").
			Through("user_blocked_groups", UserBlockedGroup.Type),
