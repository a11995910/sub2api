...
type Group struct {
	...
	// 缓存命中率累计状态的时间半衰期（天）；历史权重按时间指数衰减
	CacheHitHalfLifeDays float64 `json:"cache_hit_half_life_days,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldOauthPoolVisible:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field oauth_pool_visible", values[i])
			} else if value.Valid {
				_m.OauthPoolVisible = value.Bool
			}
		...
		case group.FieldCacheHitHalfLifeDays:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field cache_hit_half_life_days", values[i])
			} else if value.Valid {
				_m.CacheHitHalfLifeDays = value.Float64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("oauth_pool_visible=")
	builder.WriteString(fmt.Sprintf("%v", _m.OauthPoolVisible))
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("cache_hit_half_life_days=")
	builder.WriteString(fmt.Sprintf("%v", _m.CacheHitHalfLifeDays))
	builder.WriteString(", ")
