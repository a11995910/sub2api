...
type UserQuery struct {
	...
	withCheckinRecords        *CheckinRecordQuery
...
func (_q *UserQuery) QueryBlockedGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryCheckinRecords chains the current query on the "checkin_records" edge.
func (_q *UserQuery) QueryCheckinRecords() *CheckinRecordQuery {
	query := (&CheckinRecordClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(checkinrecord.Table, checkinrecord.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.CheckinRecordsTable, user.CheckinRecordsColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *UserQuery) QueryUserBlockedGroups() *UserBlockedGroupQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCheckinRecords:        _q.withCheckinRecords.Clone(),
...
func (_q *UserQuery) WithBlockedGroups(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithCheckinRecords tells the query-builder to eager-load the nodes that are connected to
// the "checkin_records" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithCheckinRecords(opts ...func(*CheckinRecordQuery)) *UserQuery {
	query := (&CheckinRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCheckinRecords = query
	return _q
}
...
func (_q *UserQuery) WithUserBlockedGroups(opts ...func(*UserBlockedGroupQuery)) *UserQuery {
	query := (&UserBlockedGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBlockedGroups; query != nil {
		if err := _q.loadBlockedGroups(ctx, query, nodes,
			func(n *User) { n.Edges.BlockedGroups = []*Group{} },
			func(n *User, e *Group) { n.Edges.BlockedGroups = append(n.Edges.BlockedGroups, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withCheckinRecords; query != nil {
		if err := _q.loadCheckinRecords(ctx, query, nodes,
			func(n *User) { n.Edges.CheckinRecords = []*CheckinRecord{} },
			func(n *User, e *CheckinRecord) { n.Edges.CheckinRecords = append(n.Edges.CheckinRecords, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withUserBlockedGroups; query != nil {
		if err := _q.loadUserBlockedGroups(ctx, query, nodes,
			func(n *User) { n.Edges.UserBlockedGroups = []*UserBlockedGroup{} },
			func(n *User, e *UserBlockedGroup) { n.Edges.UserBlockedGroups = append(n.Edges.UserBlockedGroups, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadBlockedGroups(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*User)
	nids := make(map[int64]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.BlockedGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(user.BlockedGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.BlockedGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.BlockedGroupsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *UserQuery) loadCheckinRecords(ctx context.Context, query *CheckinRecordQuery, nodes []*User, init func(*User), assign func(*User, *CheckinRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(checkinrecord.FieldUserID)
	}
	query.Where(predicate.CheckinRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CheckinRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadUserBlockedGroups(ctx context.Context, query *UserBlockedGroupQuery, nodes []*User, init func(*User), assign func(*User, *UserBlockedGroup)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userblockedgroup.FieldUserID)
	}
	query.Where(predicate.UserBlockedGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserBlockedGroupsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
