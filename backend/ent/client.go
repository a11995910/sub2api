...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.ChannelMonitorHistory, c.ChannelMonitorRequestTemplate, c.CheckinRecord,
...
// QueryUser queries the user edge of a CheckinRecord.
func (c *CheckinRecordClient) QueryUser(_m *CheckinRecord) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(checkinrecord.Table, checkinrecord.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
...
// QueryCheckinRecords queries the checkin_records edge of a User.
func (c *UserClient) QueryCheckinRecords(_m *User) *CheckinRecordQuery {
	query := (&CheckinRecordClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
...
type (
	hooks struct {
		...
		ChannelMonitorRequestTemplate, CheckinRecord, ErrorPassthroughRule, Group,
