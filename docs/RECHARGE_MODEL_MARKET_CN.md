...
- 视频价格在分组启用独立视频倍率时使用视频倍率，否则使用用户有效文本倍率。视频输出报价依次读取当前分组的分辨率覆盖价、渠道分辨率层级价、渠道默认价和已知 Grok 模型系统默认价；请求携带参考图时，再叠加渠道 `input_price` 或模型官方默认的按张附加价。
...
视频模型卡片按官方能力展示输出分辨率：`grok-imagine-video` 展示 `480p / 720p`，`grok-imagine-video-1.5` 展示 `480p / 720p / 1080p`，并单独展示“参考图附加 / 张”。分组分辨率覆盖价、显式 `billing_mode=video` 的渠道层级价和渠道默认价均按秒展示；历史 `billing_mode=image/per_request` 的渠道价按次展示，不会套用按秒总价公式。`billing_mode=token` 不作为视频价格来源；历史按次模式缺少当前分辨率层级价和默认价时也不回退系统每秒价，避免把不同单位拼接为同一报价。
