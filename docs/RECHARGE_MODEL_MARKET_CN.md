...
如果管理端开启邀请返利并配置了“支付奖励分组”和“支付奖励天数”，被邀请用户每完成一笔内置支付订单，系统还会为邀请人发放对应分组的限时使用权。奖励分组可以是标准专属分组或订阅分组：标准专属分组只授予访问权限，请求仍按该分组倍率正常消耗邀请人的余额；订阅分组则复用订阅续期规则，按订阅额度消耗。邀请人已有未过期限时授权时会从当前过期时间继续累加天数，已有但已过期时从当前时间重新计算；如果管理员已经永久授予该专属分组，则奖励不会覆盖永久授权。用户侧 `/affiliate` 邀请返利页面会用醒目的奖励卡片展示当前可获得的支付奖励分组、奖励天数和消耗方式；当邀请人已经拥有该奖励的限时授权或订阅时，卡片会显示当前剩余倒计时。用户侧 `/groups/available` 会返回 `access_expires_at` 和 `access_source`，API Key 创建/编辑和行内切换分组下拉会据此显示限时授权剩余时间；公开分组、永久授权和未携带到期时间的分组不显示倒计时。订单履约会写入 `AFFILIATE_GROUP_ACCESS_REWARD_APPLIED` 或 `AFFILIATE_GROUP_ACCESS_REWARD_SKIPPED` 审计记录，保证同一支付订单重试时不会重复累加天数。标准专属分组奖励到期后，后台任务会把该用户仍绑定在奖励分组上的 API Key 迁移到 `API Key 默认分组`，并移除这条限时授权；未配置有效默认分组时会暂缓过期清理，避免 Key 失去可用分组。
