...
如果管理端开启邀请返利并配置了“支付奖励分组”和“支付奖励天数”，被邀请用户每完成一笔内置支付订单，系统还会为邀请人发放对应订阅分组的使用权。奖励复用订阅续期规则：邀请人没有该分组订阅时新建订阅；已有且未过期时从当前过期时间继续累加天数；已有但已过期时从当前时间重新计算。订单履约会写入 `AFFILIATE_SUBSCRIPTION_REWARD_APPLIED` 或 `AFFILIATE_SUBSCRIPTION_REWARD_SKIPPED` 审计记录，保证同一支付订单重试时不会重复累加天数。订阅过期后会按现有订阅校验自然失效，用户未指定有效订阅分组时继续回到默认分组调度。
