...
- 渠道接口为每个模型返回持久号池实际支持的 `group_ids`；模型广场只把模型放入这些分组，不再把同平台模型与全部分组做笛卡尔组合。
...
- 后端测试：`go test ./...`
- 部署后检查 `/purchase` 会跳转到 `/redeem`，并检查 `/payment`、`/orders`、`/redeem`、`/subscriptions`、`/model-market`、`/model-test`、管理端用户分组配置、已有 Key 的黑名单鉴权、邀请返利记录和用户侧菜单显示。
