...
本功能只服务于站内“模型测试台”的视频测试。普通开发者直接调用 `/v1/videos`、`/videos` 或对应状态接口时，不会创建模型测试台记录，OpenAI 兼容响应格式也不变；普通余额计费异步任务使用独立的 `video_task_billings` 账单闭环，见 `VIDEO_TASK_BILLING_CN.md`。
...
- 删除只移除模型测试台本地记录，不取消上游任务，也不代表普通任务退款或释放冻结。
