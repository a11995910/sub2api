...
292/332 门票按账号与模型隔离。启用缺票阻断时，票据失效或模型声明不一致后，该账号的对应模型退出调度，后台独立重新采集。OpenAI 文本入口（Responses、Chat Completions、Messages）在准入前、排队抢槽时和抢槽成功后复核票据；排队复核沿用并发轮询间隔（100 毫秒至 2 秒），发现缺票后释放槽位与等待计数，在本次请求的换号预算内尝试其他可用账号，不等待采集完成。候选账号或换号预算耗尽时返回错误，准入阶段的缺票错误保留 `503 / turn_state_unavailable` 语义；若仅已发送 SSE 心跳，则使用流内错误终止。已经输出业务响应的请求不因后台采票而自动重放。
