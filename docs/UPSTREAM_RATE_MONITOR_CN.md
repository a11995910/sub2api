...
上游倍率监控用于维护多个同类 Sub2API 上游站点，并通过后台账号登录上游读取分组列表，缓存最近一次分组倍率快照。管理员可以在后台实时查看：
...
- `username`：上游后台登录账号。
- `password_encrypted`：上游后台登录密码密文，使用项目现有 `SecretEncryptor` 加密。
- `enabled`：是否启用。
...
3. 使用该 token 分页请求上游 `/api/v1/admin/groups`。
4. 读取 `items` 中的分组字段，写入 `last_snapshot`。

上游接口要求与本项目 Sub2API 管理端接口保持兼容，至少需要返回：

- 登录接口：`data.access_token`。
- 分组列表接口：`data.items`，其中分组字段包含 `name`、`rate_multiplier` 等。
...
- 上游登录需要 2FA 时按失败处理，需要改用无需 2FA 的管理账号或调整上游登录策略。
