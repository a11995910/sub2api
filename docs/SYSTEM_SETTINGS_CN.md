...
管理员在后台 `系统设置 > 功能开关` 中维护渠道监控开关、运行模式、默认间隔以及用户侧展示策略。配置通过 `settings` 表的 key/value 记录保存。

| 配置项 | settings key | 说明 |
| --- | --- | --- |
| 启用状态 | `channel_monitor_enabled` | 控制渠道监控管理入口、用户只读入口和后台任务是否可用。 |
| 运行模式 | `channel_monitor_mode` | `v1` 使用主动探活与配额监控；`v2` 使用被动聚合视图。缺失或非法值回退 `v1`。 |
| 默认检测间隔 | `channel_monitor_default_interval_seconds` | 新建主动监控使用的默认间隔，实际监控配置仍需满足 `15~3600` 秒范围。 |
| 隐藏吞吐量 | `channel_monitor_hide_throughput` | 控制用户侧 V2 视图是否隐藏 RPM/TPM 规模信号，管理端不受影响。 |
| 展示配额与余额 | `channel_monitor_show_quota` | 控制用户侧 V1 监控是否返回和展示关联账号的配额/余额快照，默认关闭；管理端始终可见。 |

`channel_monitor_show_quota` 采用失败关闭策略，只有字面值 `true` 才开启。关闭时服务端会从用户接口响应中剥离 `latest_quota`，不是只依赖前端隐藏；公开设置和 HTML 注入仍返回该布尔值，供用户界面决定是否渲染配额组件。完整监控流程见[渠道监控功能说明](CHANNEL_MONITOR_CN.md)。

配置读取与保存入口：

| 入口 | 说明 |
| --- | --- |
| `GET /api/v1/admin/settings` | 管理端读取完整渠道监控设置。 |
| `PUT /api/v1/admin/settings` | 管理端保存渠道监控设置。 |
| `GET /api/v1/settings/public` | 返回用户端需要的监控开关、模式和展示字段。 |
