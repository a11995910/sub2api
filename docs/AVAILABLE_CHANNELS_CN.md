...
- 启用 `cache_hit_quarter_to_input_enabled` 的分组，只会处理 OpenAI 入站协议的 HTTP SSE 流式 `Chat Completions` 和 `Responses` 请求，并且必须正常产生成功终止事件及 usage。Anthropic Messages 等其他入站协议、非流式请求、Responses WebSocket、`/responses/compact`、`/responses/input_tokens`、图片和视频接口即使配置了该开关也不生效；失败、`incomplete`、仅由 EOF 合成终止、首帧就是终态且此前没有内容成功下发，或在处理成功终态前已经检测到客户端断连的流同样不划拨。
- 对符合范围的请求，系统按“用户 + 分组 + 目标 + 容差 + 半衰期 + 状态代次”累计提示词 token 和调整后缓存读取 token。在成功终止 usage 写给下游前，系统先按 `2^(-经过秒数/半衰期秒数)` 同比衰减两项历史权重，再加入本次请求。加入后未超过“目标 + 容差”时不划拨；超过后只把本次请求实际拥有且为回到目标所必需的缓存读取 token 划入普通输入，不会回溯改写历史请求。低命中时期的影响会随时间衰减，因此后续高命中流量可以逐步恢复划拨；半衰期越短，恢复越快。
- 累计状态通过 Redis 原子脚本更新，多个 API Key、多个账号和并发请求共享同一用户分组口径。分组保存后 `updated_at` 形成新的状态代次，新请求不再继承旧配置的累计值；当前功能使用 `cache_hit_target:v4:openai_stream:` 状态前缀，不继承 `v3` 及更早版本的累计值。每次更新都携带该次 tracker 调用内稳定的 operation ID，Redis 会在 10 分钟内保存其首次完整结果；Redis 客户端因网络不确定而内部重试同一条命令时直接回放首次结果，不会重复推进累计状态。下游重新发起的 HTTP 请求仍是新的计费请求，不与前一次合并。累计状态采用 30 天滑动过期，但 30 天只表示连续无请求后的状态清理期限，不是统计周期。
- Redis tracker 不可用或返回错误时，本次请求跳过划拨：下游 usage 和本站计费都保留原值，响应仍继续，不使用单请求估算替代累计结果。
- 每个符合条件的请求只执行一次划拨：下游收到的流式 usage 与本站统计、账单、余额、订阅额度、API Key 配额和账号配额共用同一次划拨结果；兼容上游重复发送成功终态时复用首份权威 usage 快照，不会再次推进累计状态。OpenAI usage 的顶层 `input_tokens`、`prompt_tokens` 和 `total_tokens` 保持不变，只降低缓存读取明细；本站计费消费响应阶段生成的同一份划拨快照。实际发生划拨时，Responses usage 中可能携带划拨前缓存明细的非标准 `attribution` 字段不会向下游透传，其他标准 usage 字段保持不变。真实上游缓存不变，历史用量不回填。
- 划拨发生在成功终态 usage 准备写给下游时，因此不会增加首字时间；它可能增加最后一个 usage 帧的完成延迟。Redis 更新与终态网络写出无法组成同一原子事务：若终态改写完成后连接恰好在写出时失败，累计状态可能已经推进，这是当前保留的极窄边界。
