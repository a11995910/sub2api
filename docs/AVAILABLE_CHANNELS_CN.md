...
`grok-imagine-video-1.5` 在没有参考图的文生视频请求中会按标准模型 `grok-imagine-video` 路由和计费；上传参考图执行图生视频时才按 `grok-imagine-video-1.5` 计费。视频模式渠道定价的 `input_price` 表示每张参考图附加价；为空时标准版使用官方 `$0.002/张`，1.5 使用官方 `$0.01/张`，显式 `0` 表示参考图免费。模型广场独立展示每个模型支持的输出分辨率和参考图附加价；测试台根据是否上传参考图切换实际计费模型并显示费用拆分。标准版只展示 `480p/720p`，不为官方未提供的 `1080p` 生成伪报价。
