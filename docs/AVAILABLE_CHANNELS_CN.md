...
- 已配置有效的 API Key 默认分组：系统会先把这些 Key 批量迁移到默认分组，再删除原分组。
- 未配置默认分组：系统会拒绝删除，避免 Key 继续指向已删除分组后静默失效。
- 默认分组就是当前要删除的分组：系统会拒绝删除，管理员需要先改成其他启用分组。
- 默认分组不存在或已停用：系统会拒绝删除，管理员需要重新选择启用状态的分组。
