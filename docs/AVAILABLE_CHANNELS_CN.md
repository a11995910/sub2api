...
- 视频计费模型：若 `video_rate_independent=true`，使用 `video_rate_multiplier`；否则使用当前有效分组倍率；总价为“分辨率每秒价格 × 视频时长 + 参考图按张附加价 × 实际参考图数量”，两部分使用同一有效视频倍率。
...
`grok-imagine-video-1.5` 在没有起始图的文生视频请求中会按标准模型 `grok-imagine-video` 路由和计费；上传起始图执行图生视频时才按 `grok-imagine-video-1.5` 计费。测试台的起始图入口只对 1.5 系列开放，超过 1MB 的图片会提示并在浏览器压缩后提交为 `image.url`；标准版的 `reference_images[].url` 属于独立的参考图引导工作流。视频模式渠道定价的 `input_price` 表示每张参考图附加价；为空时标准版使用官方 `$0.002/张`，1.5 使用官方 `$0.01/张`，显式 `0` 表示参考图免费。模型广场独立展示每个模型支持的输出分辨率和参考图附加价；测试台根据是否上传起始图切换实际计费模型并显示费用拆分。标准版只展示 `480p/720p`，不为官方未提供的 `1080p` 生成伪报价。
