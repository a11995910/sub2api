...
- 每个符合条件的请求只执行一次划拨：下游收到的流式 usage 与本站统计、账单、余额、订阅额度、API Key 配额和账号配额共用同一次划拨结果；兼容上游重复发送成功终态时复用首份权威 usage 快照，不会再次推进累计状态。OpenAI usage 的顶层 `input_tokens`、`prompt_tokens` 和 `total_tokens` 保持不变，只降低缓存读取明细；本站计费消费响应阶段生成的同一份划拨快照。真实上游缓存不变，历史用量不回填。
