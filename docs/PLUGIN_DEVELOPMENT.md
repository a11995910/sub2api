...
本文面向希望为 Sub2API 开发、打包和发布插件的团队。插件由独立进程和静态 UI
组成，交付为 `.s2plugin`，宿主通过稳定的 gRPC 协议调用。本文以当前宿主已经定义的
`openai.oauth.outbound_transport.v1` 能力作为协议示例。

本文不是一个可直接安装的完整插件，也不代表 Sub2API 已经发布对应的官方
`.s2plugin` 包。当前文档主要描述公开协议、宿主边界和开发流程。涉及用户身份、
用量账本或余额事务的功能只有在宿主提供明确的受控能力后才能插件化；否则应作为
内置功能实现，插件不得直接连接宿主数据库或持有宿主凭据。

## 插件工作区与版本控制

仓库内插件源码统一放在 `plugins/`，每个插件拥有独立子目录和 `dist/`。源码和
非敏感构建产物都属于项目交付内容，不得整体加入 `.gitignore` 或
`.dockerignore`；各插件的 `dist/` 也必须保留在 Git 和 Docker 构建上下文中。
`.env`、签名私钥、Token、代理凭据和其他运行时秘密仍必须忽略。

插件按本文后续章节生成 `.s2plugin`。在宿主尚未实现对应安装协议或能力时，不得
生成一个普通 ZIP 并声称它可由插件管理页安装。
...
需要账号目录的插件通过 `InitHostServices` 获取宿主反向服务。宿主优先提供
HostService API v2；如果插件返回 `ready: false`，宿主会使用同一 broker
再尝试兼容的 v1。v2 在 v1 的账号 ID 列表之外增补账号元数据，保留 v1 的
账号查询与身份解析接口；协商版本不会扩大清单能力授予的账号访问范围。
RPC 错误、超时或插件未实现该方法时不重试；两次都拒绝时插件无法使用宿主
账号服务。Sub2API 版本范围兼容和插件进程健康不代表账号接口已连接，安装
后还应核实配置页的账号列表。
