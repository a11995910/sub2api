...
Configure an S3-compatible object store (AWS S3, Cloudflare R2, Aliyun OSS, MinIO, …) in `config.yaml` (all keys also accept the `IMAGE_STORAGE_*` environment overrides). Docker deployments pass through `IMAGE_STORAGE_ENABLED`, `IMAGE_STORAGE_ENDPOINT`, `IMAGE_STORAGE_REGION`, `IMAGE_STORAGE_BUCKET`, `IMAGE_STORAGE_ACCESS_KEY_ID`, `IMAGE_STORAGE_SECRET_ACCESS_KEY`, `IMAGE_STORAGE_PREFIX`, `IMAGE_STORAGE_FORCE_PATH_STYLE`, `IMAGE_STORAGE_PUBLIC_BASE_URL`, `IMAGE_STORAGE_PRESIGN_EXPIRY_HOURS`, and `IMAGE_STORAGE_MAX_DOWNLOAD_BYTES`:
