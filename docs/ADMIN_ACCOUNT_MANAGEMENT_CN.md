...
OpenAI OAuth 账号编辑页提供互斥的状态头模式：关闭、健康头异常补试、健康头首发注入、上游 292 门票。未设置模式的历史账号按原替换开关回显为异常补试或关闭。新建独立 OpenAI `oauth`／`setup-token` 账号未提供策略或旧开关时，默认关闭状态头策略，与订阅档位无关；显式模式或旧开关优先，已有账号和影子账号不追溯更改。普通业务请求不再记录健康头，旧记录开关不再生效。开启替换后，复用全局动态 IP 提取接口，并设置每个实际模型需要保留的健康头数量，新账号默认 2 个；已有账号可在编辑页点击“设为每模型 4 个”后，通过底部“更新”保存。系统会全局记住最近一次主动修改并保存的模型选择；新账号和未保存模型选择的旧账号默认沿用其中自身上游支持的模型，已有独立选择的账号不受后续默认变化影响。打开弹窗只预览，不会写入账号；未手动保存时，由后台首次维护核实并固定本账号选择。模型复选框从该账号上游 Codex 目录同步，过滤媒体模型和不受账号映射支持的项；没有历史选择、同步失败或没有支持交集时明确提示，不猜测模型、不自动全选。映射到相同实际模型的别名共用一份目标库存。

健康头只通过动态接口提取的临时代理采集，不改业务代理。服务端每 15 秒检查库存：有效头（包含正在使用的头）达到目标后，不再提取代理或发送测试；使用失败或 40 分钟到期时清除可用内容，并补齐缺口。普通替换成功归还原头，不延长有效期。服务端维护不依赖弹窗，关闭页面或服务重启后仍依据保存配置继续；关闭替换开关停止后续维护。仅开启替换、账号有效且全局接口可用时才执行自动继承与采集；继承失败不会保存空模型配置，等待下一轮核实。

成功采集后临时记住该代理入口，同账号、实际模型及传输方式的后续采集优先复用，最近成功后 10 分钟有效；再次采集失败即舍弃优先资格，下一步换入口。缓存仅在进程内，重启或保存对应采集配置后清理，共享提取配置变更清理全部缓存。缓存的是包含协议和端口的供应商入口，无法保证其背后的实际出口 IP 固定；凭据不在界面或日志展示。满库存时仍停止采集，复用次数仍受每轮尝试上限约束。

后台最多同时维护 8 个账号，每账号最多 10 路并行探测，全局最多 24 路，账号之间轮流分配采集机会；每账号每轮最多 30 次探测，并在批次之间检查 90 秒时间片，到限后释放名额等待下一轮。每批根据实际模型缺口预留采集数量，避免并行超采。连续 10 次没有健康结果后暂停，批次间至少间隔 1 秒，并轮换仍有缺口的模型；接口提取失败或没有新入口也计入连续失败。成功采集或返回已有的有效健康头会清零连续失败次数，重复头不增加库存。达到阈值后固定等待 15 秒再进入后续扫描，不再逐次延长等待；实际重启时间还受 15 秒扫描间隔和最多 8 个并行维护账号影响。连续失败按结果完成顺序计算，达到阈值后停止后续批次，已派发的本批请求仍正常收尾。每轮最多尝试入口数默认 100，后台另受上述 30 次与时间片限制，取先到达者；手动运行使用配置上限，数量不足 10 时也不会越过上限。模型目录、配置或库存读取失败时显示原因并等待下一轮，不盲目更换代理。采集失败不会删除此前保存的有效头。

动态接口支持公网 HTTP／HTTPS，完整地址全局加密保存，界面只显示脱敏地址。提取 URL 和代理协议全局填写一次，其他账号直接复用；地址留空保留已有值，填写新地址或修改代理协议会更新所有账号使用的配置。仅修改本账号模型、数量或传输方式时，不覆盖全局代理地址和协议；这些采集选项和开关仍按账号独立保存。直接保存继承结果或只改数量、传输方式，不会修改全局最近模型选择。供应商可返回换行 `IP:端口`，也支持 BestGo 的 `code=200`、`success="success"`、`data=[{"ip":"公网IP","port":端口}]` JSON；支持 HTTP／HTTPS／SOCKS5H 代理；白名单须添加 Sub2API 服务器实际出口 IP。供应商提取参数原样传递，入口数量不等于不同出口 IP 数量。每次探测只发送一次 `hi`，要求 15 秒内有有效输出且响应完整成功，并返回有效状态头；失败或重复头不补充库存，采集不占用业务并发槽，也不恢复账号错误或冷却状态。

面板显示有效库存、使用中数量、历史累计、维护结果及下次重试时间；有缺口但尚未调度时显示“排队待采集”，配置或库存检查失败时显示“采集异常”，活跃时每 15 秒刷新，提取接口 403 或白名单失败等原因会明确显示。采集配置与替换开关通过底部“更新”一起保存。关闭弹窗不会停止已保存的自动维护，停止后续补位需关闭替换开关并保存。健康头按账号与映射后实际模型隔离，HTTP／WebSocket 和不同出口共用同一范围；异常补试模式只使用匹配的健康头补试一次；首发模式在请求发出前领取，失败即淘汰且不再叠加健康头补试。空闲头剩余不足十分钟时提前采集替换，采集失败继续保留原头至其本地期限。历史累计在过期或淘汰后保留。管理端统计接口不返回状态头内容，完整接口与替换规则见 [健康状态头采集与替换](OPENAI_GATEWAY_CN.md#健康状态头采集与替换)。

通过管理员 API Key 推送 OAuth 时，在 `POST /api/v1/admin/accounts` 的原有账号创建请求中合并以下字段即可。管理员认证使用请求头 `x-api-key`，账号名称、平台、类型和 OAuth 凭据继续沿用现有请求格式：
...
这只是需要增加的字段，不是完整创建请求。预先保存过全局动态 IP 接口和至少一次模型选择后，后台会自动为该账号核实模型交集、保存独立配置并开始维护，每模型默认 2 个；无需打开编辑页或额外调用动态配置 `PUT`。批量创建中每个账号的 `extra`、`POST /api/v1/admin/accounts/import/codex-session` 的顶层 `extra` 也接受同一开关。导入更新已有账号时保留其独立模型选择；开关必须为 JSON 布尔值 `true`，不能传字符串 `"true"`。

新建独立 OpenAI OAuth／SetupToken 账号省略策略时默认写入 `extra.openai_turn_state_mode=off`，并关闭兼容健康头开关；普通创建、批量创建、Codex 导入新建及 CRS 同步新建均遵循此规则。继续使用健康头异常补试或首发注入时需显式传上述旧开关或 `extra.openai_turn_state_mode=healthy_retry`／`healthy_preflight`；关闭所有策略可传 `off`。导入更新、后续档位变更及历史账号读取不会套用新建默认，完整导入数据中已显式保存的旧策略仍保留。开启健康头策略仍须配置其动态 IP 接口并核实模型选择。292 门票需显式选择 `codex_ticket`，使用独立的总开关与采集来源。

OpenAI 账号编辑页提供“请求完整性观察”，默认“观察”，可以按账号关闭；设置保存于 `extra.request_integrity_mode`，仅接受 `observe`、`off`。它记录 Responses 转发中变化的字段名，不记录对话正文，不阻断请求，也不更改“Codex 指纹收敛”选择。覆盖范围、跳过条件与日志含义见 [OpenAI 网关功能说明](OPENAI_GATEWAY_CN.md#请求完整性观察)。

手动账号测试、定时测试和手动状态头采集均可在账号限流、过载、临时暂停或模型冷却期间发送，不占用业务账号并发槽。测试仍使用现有认证、模型及出口配置，实际可用性以上游响应为准。

普通手动账号测试成功后，按账号当前状态恢复错误、账号限流、过载、临时暂停和模型限流，并清理相关运行时缓存。该恢复不再比较测试开始时的行版本，因此也可能清除测试期间新写入的限制；测试返回的用量和能力观测直接保存。管理员调度开关和配额配置由各自的管理入口维护。手动状态头采集仅保存健康头，不执行普通账号测试的状态恢复。

定时测试的 `AutoRecover` 开启且测试成功时，执行同样的状态恢复；关闭时仅保存测试结果与观测。手动“恢复状态”和“重置错误”仍提供独立管理入口。
...
- 健康头默认模型测试覆盖最近手动选择的保存、上游支持交集、已有账号配置隔离、继承失败后重试，以及仅通过管理员 API 开启替换后无需访问页面即可完成初始化与补充。
- 服务与组件测试覆盖国产供应商模式/协议组合、默认端点、额度和余额响应解析、代理与出站 URL 校验、并发探测合并、快照落盘失败以及不支持的供应商组合。

### 状态头模式对比

账号 `extra.openai_turn_state_mode` 可选 `off`、`healthy_retry`、`healthy_preflight`、`codex_ticket`。选择健康头模式时展示原动态 IP 采集配置和库存，首发模式默认缺头暂停该账号的对应模型，由调度尝试其他可用账号；显式关闭缺头暂停时继续原请求。同一请求／WS 会话中，每个账号各有一次健康头使用机会，换号可使用新账号自己的头，回到原账号不会重置次数；总换号上限和输出后不可重放规则保持有效。292 门票模式默认同时兼容 292 和 332 字符的状态头，仍要求 HTTP 200 和 `gAAAAA` 前缀，账号摘要显示实际门票长度。该模式使用系统设置中的独立采集来源与总开关，可选择固定代理地址或批量 IP 提取接口，配置独立于健康头动态 IP 接口。批量接口每轮提取一次，最多四个账号／模型目标并行，各目标逐个尝试代理，取得有效票即停，每账号模型保留一张票及成功代理，后续业务持续使用这一组绑定，直到门票过期或绑定失效后重新采集。供应商必须使用固定或粘性出口（关闭提前换 IP），粘性时长应覆盖门票期限；轮换入口不能保证实际 IP 一致。旧票没有代理信息时会重新采集，重启可恢复完整绑定。账号摘要每五秒显示就绪状态、剩余期限、采集中及批次进度、最近安全结果和预计下次尝试时间；编辑中的未保存字段不会被轮询覆盖。默认缺票暂停，可按账号关闭缺票拦截。模型列表、长度和期限在 `gateway.openai_codex_ticket` 配置；`refresh_before_seconds` 仅保留兼容，不再提前换票和换出口。

模式切换后只运行对应采集器及注入器，旧库存保留但不跨策略使用。健康头历史使用／成功／失败不随模式切换清零；实际对比应使用独立账号／分组和固定时间窗口的请求统计。完整行为和边界见[状态头模式与对比](OPENAI_GATEWAY_CN.md#状态头模式与对比)。
