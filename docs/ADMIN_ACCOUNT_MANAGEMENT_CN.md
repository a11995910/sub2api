...
OpenAI OAuth 账号编辑页只保留“异常时替换健康状态头（实验）”，默认关闭；普通业务请求不再记录健康头，旧记录开关不再生效。开启替换后，填写并保存动态 IP 提取接口、勾选上游支持的测试模型，并设置每个实际模型需要保留的健康头数量，默认 3 个。模型复选框从该账号上游 Codex 目录同步，过滤媒体模型和不受账号映射支持的项；同步失败明确提示重试，不猜测默认模型。映射到相同实际模型的别名共用一份目标库存。
...
动态接口必须是 HTTPS，完整地址按账号加密保存，界面只显示脱敏地址；地址留空保留已有值。供应商返回换行 `IP:端口`，支持 HTTP／HTTPS／SOCKS5H 代理；白名单须添加 Sub2API 服务器实际出口 IP。供应商提取参数原样传递，入口数量不等于不同出口 IP 数量。每次探测只发送一次 `hi`，要求 5 秒内有有效输出且响应完整成功，并返回有效状态头；失败或重复头不补充库存，采集不占用业务并发槽，也不恢复账号错误或冷却状态。
