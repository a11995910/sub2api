...
OpenAI 账号编辑页提供“记录健康状态头”（默认关闭）和“异常时替换健康状态头（实验）”（默认关闭），两个开关独立保存。开启记录可收集 Responses 有有效输出且完整成功返回的 `X-Codex-Turn-State`；开启替换后，在 HTTP 请求或 WS 握手返回 429／503 时用同账号、同模型、同传输方式的记录补试一次，仍失败就淘汰。状态加密持久化到 PostgreSQL，最长允许使用 40 分钟，重启保留；失败头清除内容，保留使用统计。关闭记录不影响剩余且范围匹配的旧记录的使用。完整范围和等待、并发、失败处理规则见 [健康状态头记录与替换](OPENAI_GATEWAY_CN.md#健康状态头记录与替换)。

两个开关旁可手动获取健康状态头：选择模型（默认 `gpt-5.6-sol`）、HTTP／WebSocket 和最多 5 个测试出口，逐个发送 `hi`，仅完整健康且返回状态头时记录；429／503 或其他失败不记录、不重试。界面显示各出口结果和过期时间，可取消后续测试。测试使用已保存的账号配置，可在账号或模型冷却期间发送，且不占用业务并发槽；不保存当前编辑表单、不修改账号代理；手动采集可在自动记录关闭时使用。记录仍按出口和传输方式隔离。旁边的统计区可刷新查看可尝试数量、累计采集、替换成功／失败与成功率，以及最近的记录和手动采集结果。未过期不等于已验证可恢复；只有替换后有有效输出且完整结束才计成功，未确认结果不进入成功率。统计接口为管理员专用 `GET /api/v1/admin/accounts/:id/healthy-turn-state`，不返回状态头内容。
