...
OpenAI OAuth 账号编辑页提供互斥的状态头模式：关闭、健康头异常补试、健康头首发注入、上游 292 门票。未设置模式的历史账号按原替换开关回显为异常补试或关闭。新建 OpenAI `oauth` 账号的档位已识别为 Business Premium 时，未提供开关则默认开启；显式关闭优先，其他档位和 `setup-token` 仍默认关闭，已有账号不追溯更改。普通业务请求不再记录健康头，旧记录开关不再生效。开启替换后，复用全局动态 IP 提取接口，并设置每个实际模型需要保留的健康头数量，默认 3 个。系统会全局记住最近一次主动修改并保存的模型选择；新账号和未保存模型选择的旧账号默认沿用其中自身上游支持的模型，已有独立选择的账号不受后续默认变化影响。打开弹窗只预览，不会写入账号；未手动保存时，由后台首次维护核实并固定本账号选择。模型复选框从该账号上游 Codex 目录同步，过滤媒体模型和不受账号映射支持的项；没有历史选择、同步失败或没有支持交集时明确提示，不猜测模型、不自动全选。映射到相同实际模型的别名共用一份目标库存。
...
新建 Business Premium OAuth 时可以省略上述开关，前提是创建请求中的 `credentials.plan_type` 已正确识别为 `self_serve_business_prolite`；匹配兼容大小写、空格、下划线和连字符差异。`team` 表示 Business Standard，`prolite` 表示 Pro 5x，两者均不自动开启。需要关闭时显式传 `extra.openai_healthy_turn_state_replace=false`。此规则只应用于新建 OpenAI `oauth` 账号，导入更新或后续变更档位不会改动已有开关；默认开启也仍需全局动态 IP 接口与可继承模型配置才能开始采集。
...
账号 `extra.openai_turn_state_mode` 可选 `off`、`healthy_retry`、`healthy_preflight`、`codex_ticket`。选择健康头模式时展示原动态 IP 采集配置和库存，首发模式可选择缺头暂停该模型，默认缺头仍继续原请求。292 门票模式使用系统设置中的独立旋转代理与总开关，不要求保存健康头动态 IP 接口；账号摘要显示各模型就绪状态和剩余期限，默认缺票暂停，可按账号关闭缺票拦截。模型列表、长度、期限和刷新窗口在 `gateway.openai_codex_ticket` 配置。
