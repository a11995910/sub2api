...
这只是需要增加的字段，不是完整创建请求。预先保存过全局动态 IP 接口和至少一次模型选择后，后台会自动为该账号核实模型交集、保存独立配置并开始维护，每模型默认 4 个；无需打开编辑页或额外调用动态配置 `PUT`。批量创建中每个账号的 `extra`、`POST /api/v1/admin/accounts/import/codex-session` 的顶层 `extra` 也接受同一开关。导入更新已有账号时保留其独立模型选择；开关必须为 JSON 布尔值 `true`，不能传字符串 `"true"`。
