...
新建独立 OpenAI OAuth／SetupToken 账号省略策略时默认写入 `extra.openai_turn_state_mode=codex_ticket`，并关闭旧健康头开关；普通创建、批量创建、Codex 导入新建及 CRS 同步新建均遵循此规则。继续使用健康头异常补试时需显式传上述旧开关或 `extra.openai_turn_state_mode=healthy_retry`；关闭所有策略可传 `off`。导入更新、后续档位变更及历史账号读取不会套用新建默认，完整导入数据中已显式保存的旧策略仍保留。默认选择 292 门票不会打开全局总开关；采集仍须先完成全局采集来源配置。
