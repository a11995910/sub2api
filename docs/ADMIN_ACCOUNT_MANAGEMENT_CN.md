...
新建独立 OpenAI OAuth／SetupToken 账号省略策略时默认写入 `extra.openai_turn_state_mode=healthy_preflight`，并开启兼容健康头开关；普通创建、批量创建、Codex 导入新建及 CRS 同步新建均遵循此规则。继续使用健康头异常补试时需显式传上述旧开关或 `extra.openai_turn_state_mode=healthy_retry`；关闭所有策略可传 `off`。导入更新、后续档位变更及历史账号读取不会套用新建默认，完整导入数据中已显式保存的旧策略仍保留。首发模式未设置缺头策略时默认暂停本账号该模型；显式设置 `extra.openai_healthy_turn_state_fail_closed=false` 时继续原请求；健康头采集仍须配置其动态 IP 接口并核实模型选择。292 门票需显式选择 `codex_ticket`，使用独立的总开关与采集来源。
