...
OpenAI OAuth 账号编辑页只保留“异常时替换健康状态头（实验）”，默认关闭；普通业务请求不再记录健康头，旧记录开关不再生效。开启替换后，复用全局动态 IP 提取接口、勾选本账号上游支持的测试模型，并设置每个实际模型需要保留的健康头数量，默认 3 个。模型复选框从该账号上游 Codex 目录同步，过滤媒体模型和不受账号映射支持的项；同步失败明确提示重试，不猜测默认模型。映射到相同实际模型的别名共用一份目标库存。

健康头只通过动态接口提取的临时代理采集，不改业务代理。服务端每 15 秒检查库存：有效头（包含正在使用的头）达到目标后，不再提取代理或发送测试；使用失败或 40 分钟到期时清除可用内容，并补齐缺口。普通替换成功归还原头，不延长有效期。服务端维护不依赖弹窗，关闭页面或服务重启后仍依据保存配置继续；关闭替换开关停止后续维护。旧动态设置没有勾选模型时不自动采集。失败会退避重试，最多尝试入口数默认为 100，控制单次补位运行的请求数。

动态接口必须是 HTTPS，完整地址全局加密保存，界面只显示脱敏地址。提取 URL 和代理协议全局填写一次，其他账号直接复用；地址留空保留已有值，填写新地址或修改代理协议会更新所有账号使用的配置。仅修改本账号模型、数量或传输方式时，不覆盖全局配置；这些采集选项和开关仍按账号独立保存。供应商返回换行 `IP:端口`，支持 HTTP／HTTPS／SOCKS5H 代理；白名单须添加 Sub2API 服务器实际出口 IP。供应商提取参数原样传递，入口数量不等于不同出口 IP 数量。每次探测只发送一次 `hi`，要求 5 秒内有有效输出且响应完整成功，并返回有效状态头；失败或重复头不补充库存，采集不占用业务并发槽，也不恢复账号错误或冷却状态。
