...
两个开关旁可手动获取健康状态头：选择模型、HTTP／WebSocket 和最多 5 个测试出口，逐个发送 `hi`，仅完整健康且返回状态头时记录；429／503 或其他失败不记录、不重试。界面显示各出口结果和过期时间，可取消后续测试。测试使用已保存的账号配置，可在账号或模型冷却期间发送，且不占用业务并发槽；不保存当前编辑表单、不修改账号代理；手动采集可在自动记录关闭时使用。记录仍按出口和传输方式隔离。
