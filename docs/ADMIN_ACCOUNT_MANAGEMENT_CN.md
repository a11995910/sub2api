...
后台最多同时维护 8 个账号，每账号最多 3 路并行探测，全局最多 24 路，账号之间轮流分配采集机会；每账号每轮最多 15 次探测，并在批次之间检查 90 秒时间片，到限后释放名额等待下一轮。每批根据实际模型缺口预留采集数量，避免并行超采。连续 5 次没有健康结果后暂停，批次间至少间隔 1 秒，并轮换仍有缺口的模型；接口提取失败或没有新入口也计入连续失败。成功采集或返回已有的有效健康头会清零连续失败次数，重复头不增加库存。达到阈值后固定等待 15 秒再进入后续扫描，不再逐次延长等待；实际重启时间还受 15 秒扫描间隔和最多 8 个并行维护账号影响。连续失败按结果完成顺序计算，达到阈值后停止后续批次，已发出的至多 2 个请求仍正常收尾。每轮最多尝试入口数默认 100，后台另受上述 15 次与时间片限制，取先到达者；手动运行使用配置上限，数量不足 5 时也不会越过上限。模型目录、配置或库存读取失败时显示原因并等待下一轮，不盲目更换代理。采集失败不会删除此前保存的有效头。
...
这只是需要增加的字段，不是完整创建请求。预先保存过全局动态 IP 接口和至少一次模型选择后，后台会自动为该账号核实模型交集、保存独立配置并开始维护，每模型默认 10 个；无需打开编辑页或额外调用动态配置 `PUT`。批量创建中每个账号的 `extra`、`POST /api/v1/admin/accounts/import/codex-session` 的顶层 `extra` 也接受同一开关。导入更新已有账号时保留其独立模型选择；开关必须为 JSON 布尔值 `true`，不能传字符串 `"true"`。

新建独立 OpenAI OAuth／SetupToken 账号省略策略时默认写入 `extra.openai_turn_state_mode=healthy_preflight`，并开启兼容健康头开关；普通创建、批量创建、Codex 导入新建及 CRS 同步新建均遵循此规则。继续使用健康头异常补试时需显式传上述旧开关或 `extra.openai_turn_state_mode=healthy_retry`；关闭所有策略可传 `off`。导入更新、后续档位变更及历史账号读取不会套用新建默认，完整导入数据中已显式保存的旧策略仍保留。默认首发模式缺头时继续原请求；健康头采集仍须配置其动态 IP 接口并核实模型选择。292 门票需显式选择 `codex_ticket`，使用独立的总开关与采集来源。
