...
面板显示有效库存、使用中数量、历史累计、维护结果及下次重试时间，活跃时每 15 秒刷新，提取接口 403 或白名单失败等原因会明确显示。采集配置与替换开关通过底部“更新”一起保存。关闭弹窗不会停止已保存的自动维护，停止后续补位需关闭替换开关并保存。健康头按账号与映射后实际模型隔离，HTTP／WebSocket 和不同出口共用同一范围；异常时只使用匹配的健康头补试一次，仍失败就淘汰。历史累计在过期或淘汰后保留。管理端统计接口不返回状态头内容，完整接口与替换规则见 [健康状态头采集与替换](OPENAI_GATEWAY_CN.md#健康状态头采集与替换)。
