...
OpenAI 账号编辑页提供“记录健康状态头”（默认开启）和“异常时替换健康状态头（实验）”（默认关闭），两个开关独立保存。先只记录可收集 Responses 正常首字返回的 `X-Codex-Turn-State`；开启替换后，在 HTTP 请求或 WS 握手返回 429／503 时用同账号、同模型、同传输方式的记录补试一次，仍失败就淘汰。状态只在内存保留，最长 40 分钟，重启清空；关闭记录不影响剩余旧记录的使用。完整范围和等待、并发、失败处理规则见 [健康状态头记录与替换](OPENAI_GATEWAY_CN.md#健康状态头记录与替换)。

两个开关旁可手动获取健康状态头：选择模型、HTTP／WebSocket 和最多 5 个测试出口，逐个发送 `hi`，仅完整健康且返回状态头时记录；429／503 或其他失败不记录、不重试。界面显示各出口结果和过期时间，可取消后续测试。测试复用已保存的账号配置、并发及冷却保护，不保存当前编辑表单、不修改账号代理；手动采集可在自动记录关闭时使用。记录仍按出口和传输方式隔离。
...
手动账号测试和定时测试共享业务账号并发槽。并发已满、账号仍处于限流/过载/临时暂停，或所选模型仍在冷却时，测试返回明确原因，不发送探测；结束、失败或取消均释放已取得的并发槽。保护自动生效，无独立关闭开关。

成功测试只尝试恢复测试开始时观察到的错误状态、已过期账号冷却及所测模型的已过期冷却。恢复使用数据库条件更新：若测试期间账号被写入新状态，跳过恢复，保留最新状态。其他模型冷却、管理员调度开关、配额和新产生的限制均不会因这次测试被清除；不无条件删除 Redis 临停或重置 403 计数。测试自身的用量/能力观测与恢复在同一次条件更新中保存；失败或未启用自动恢复时，仅按测试开始时的行版本保存观测，避免旧测试覆盖较新的业务用量。

定时测试保留 `AutoRecover`：开启且完整测试成功时才执行上述恢复，关闭时仅保存测试结果与观测。手动“恢复状态”和“重置错误”仍是独立的管理操作，不受成功测试恢复范围替代。
