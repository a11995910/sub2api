...
后台采集连续 5 次没有健康结果后暂停，失败期间至少间隔 1 秒换下一个动态代理入口，并轮换仍有缺口的模型；接口提取失败或没有新入口也计入连续失败。成功采集或返回已有的有效健康头会清零连续失败次数，重复头不增加库存。达到阈值后固定等待 15 秒再进入后续扫描，不再逐次延长等待；实际重启时间还受 15 秒扫描间隔和最多 4 个并行维护账号影响。每轮最多尝试入口数仍优先生效，默认 100；数量不足 5 时也不会越过上限。模型目录、配置或库存读取失败时直接等待下一轮，不盲目更换代理。采集失败不会删除此前保存的有效头。
...
这只是需要增加的字段，不是完整创建请求。预先保存过全局动态 IP 接口和至少一次模型选择后，后台会自动为该账号核实模型交集、保存独立配置并开始维护，每模型默认 3 个；无需打开编辑页或额外调用动态配置 `PUT`。批量创建中每个账号的 `extra`、`POST /api/v1/admin/accounts/import/codex-session` 的顶层 `extra` 也接受同一开关。导入更新已有账号时保留其独立模型选择；开关必须为 JSON 布尔值 `true`，不能传字符串 `"true"`。
