...
OpenAI 账号编辑页提供“记录健康状态头”（默认开启，显式关闭继续生效）和“异常时替换健康状态头（实验）”（默认关闭），两个开关独立保存。开启记录可收集 Responses 有有效输出且完整成功返回的 `X-Codex-Turn-State`；开启替换后，在 HTTP 请求或 WS 握手返回 429／503，或响应模型不一致且可安全重放时，用同账号、同模型的记录补试一次，仍失败就淘汰。状态加密持久化到 PostgreSQL，最长允许使用 40 分钟，重启保留；失败头清除内容，保留使用统计。关闭记录不影响剩余且范围匹配的旧记录的使用。完整范围和等待、并发、失败处理规则见 [健康状态头记录与替换](OPENAI_GATEWAY_CN.md#健康状态头记录与替换)。

两个开关旁可手动获取健康状态头：选择模型（默认 `gpt-6-astra`）、HTTP／WebSocket 和最多 5 个测试出口，逐个发送 `hi`，仅完整健康且返回状态头时记录；429／503 或其他失败不记录、不重试。界面显示各出口结果和过期时间，可取消后续测试。测试使用已保存的账号配置，可在账号或模型冷却期间发送，且不占用业务并发槽；不保存当前编辑表单、不修改账号代理；手动采集可在自动记录关闭时使用。记录按账号与映射后实际模型隔离；同账号、同模型可跨出口和传输方式使用。旁边的统计区仅展示当前账号的累计健康头数量及各模型累计数量，过期或失败不扣减历史累计，也不受最近 50 条记录的限制。有效期内同值重复返回不重复累计。历史全局池因缺少账号归属而保留在旧表，不计入新的账号与模型统计。统计接口为管理员专用 `GET /api/v1/admin/accounts/:id/healthy-turn-state`，不返回状态头内容。

大量轮换入口可选择“动态 IP 接口”：填入该账号的 HTTPS 提取链接，选择代理协议，设置目标新增健康头数量和最多尝试入口数，再开始采集。默认目标为 5、最多尝试 100 次，达到任一条件自动停止。服务端按批提取换行 `IP:端口` 并逐个测试，已有同值记录不计入新增目标；代理入口不一定等于实际出口 IP。白名单应添加 Sub2API 服务器的实际出口 IP，供应商提取数与轮转参数在链接中设置，本系统原样传递。提取接口返回 403、空列表或格式错误时，界面会显示失败原因。

动态设置按账号独立加密保存，读取仅显示脱敏链接；链接输入留空保留已有值。开始前自动保存动态设置，不保存其他账号编辑项，也不修改业务代理。采集显示新增数、尝试入口数、提取批次和最近结果，支持手动停止；关闭弹窗或切换账号停止后续采集，已记录的头保留。该任务由页面逐步推进，服务重启后需重新开始。接口、限制和兼容规则见 [动态采集说明](OPENAI_GATEWAY_CN.md#健康状态头记录与替换)。
