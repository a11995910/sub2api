...
健康头只通过动态接口提取的临时代理采集，不改业务代理。服务端每 15 秒检查库存：有效头（包含正在使用的头）达到目标后，不再提取代理或发送测试；使用失败或 40 分钟到期时清除可用内容，并补齐缺口。普通替换成功归还原头，不延长有效期。服务端维护不依赖弹窗，关闭页面或服务重启后仍依据保存配置继续；关闭替换开关停止后续维护。仅开启替换、账号有效且全局接口可用时才执行自动继承与采集；继承失败不会保存空模型配置，后台退避后重试。最多尝试入口数默认为 100，控制单次补位运行的请求数。
