...
动态接口必须是 HTTPS，完整地址全局加密保存，界面只显示脱敏地址。提取 URL 和代理协议全局填写一次，其他账号直接复用；地址留空保留已有值，填写新地址或修改代理协议会更新所有账号使用的配置。仅修改本账号模型、数量或传输方式时，不覆盖全局代理地址和协议；这些采集选项和开关仍按账号独立保存。直接保存继承结果或只改数量、传输方式，不会修改全局最近模型选择。供应商返回换行 `IP:端口`，支持 HTTP／HTTPS／SOCKS5H 代理；白名单须添加 Sub2API 服务器实际出口 IP。供应商提取参数原样传递，入口数量不等于不同出口 IP 数量。每次探测只发送一次 `hi`，要求 15 秒内有有效输出且响应完整成功，并返回有效状态头；失败或重复头不补充库存，采集不占用业务并发槽，也不恢复账号错误或冷却状态。
