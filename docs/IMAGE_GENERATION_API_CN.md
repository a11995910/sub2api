...
## 6. OAuth 图片主控模型与 Image 2.5

支持 `gpt-image-2.5-flare`、`gpt-image-2.5-sunburst` 及其 `-2026-09-08` 版本名。账号显式模型白名单和分组图片权限仍生效。

OAuth 图片请求使用独立的 Responses 主控模型，默认 `gpt-5.6-luna`；图片模型仍写入 `image_generation.model`。可在运行环境设置 `SUB2API_IMAGES_MAIN_MODEL` 并重建容器以覆盖主控模型，无需重新编译。主控模型被上游拒绝时返回对应错误，不将图片模型误标为冷却。
