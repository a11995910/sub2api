...
OpenAI 账号提供两个独立布尔开关：`extra.openai_healthy_turn_state_record` 缺省为 `true`，`extra.openai_healthy_turn_state_replace` 缺省为 `false`。管理员可先只记录，再按账号启用实验性替换。创建、更新、额外字段更新及批量更新接口均校验字段类型，非法值返回 `400 INVALID_HEALTHY_TURN_STATE_SETTING`；不需要数据库迁移。

记录来源是 Responses 上游响应的 `X-Codex-Turn-State`，只有收到包含文本、推理内容或工具参数等有效内容的完整事件后才入池。HTTP 200、WS 握手成功、心跳、`response.created` 和空 delta 都不算首字健康。首字后流内失败、异常断流或未正常结束时撤销该记录；替换试验产生的记录须等响应正常结束再入池。JSON 响应须成功完成并有有效输出。每个账号、上游模型、传输方式、目标地址、凭据、出口和指纹模式组合保留最近一条记录；允许同一组合内跨客户端会话尝试，不跨账号或跨模型使用。

记录仅存于进程内存，不进入数据库、Redis、镜像或日志；服务重启清空。最多保存 1024 个缓存项，单个头上限 16 KiB，首次记录后最长保留 40 分钟；同一值重复返回不延长保留时间。这是本地清理策略，不代表上游承诺的有效期。关闭记录仅停止新增，替换开关仍可使用尚未过期的旧记录。

账号编辑页的两个开关旁提供“手动获取健康状态头”。可填写文本模型、选择 HTTP／WebSocket，并选择账号当前代理、直连或已保存的可用代理；一次最多选择 5 个出口，前端依次请求并逐项展示结果，支持取消。每个出口只发送一次固定的 `hi`，最长 60 秒，共享账号业务并发槽并遵守已有账号／模型冷却。使用已保存的账号配置，临时出口不写回账号；手动采集不依赖自动记录开关，也不会开启自动替换。只有完整健康响应同时包含有效输出和状态头才提交到正常转发使用的同一缓存；429、503、其他错误、空响应、断流、无状态头或取消均不新增记录，也不通过替换或普通重试掩盖失败。已有同值记录不续期。测试使用的模型、出口和传输方式须与后续转发一致，HTTP 记录不用于 WS。

管理接口为 `POST /api/v1/admin/accounts/:id/healthy-turn-state/test`，请求体接受 `model`（可省略）、`transport`（`http`／`websocket`，默认 `http`）、`proxy_id`（省略或 `null` 使用当前代理，`0` 直连，正数指定现有代理）。权限沿用管理端认证。结果返回状态、实际模型、传输方式、上游 HTTP 状态码及记录过期时间，不返回状态头原值、代理凭据或上游错误正文。
...
补试仍遇到 HTTP 错误、传输错误、空响应、流内失败、首字超时或异常 EOF 时淘汰已用记录，并在本地保留其散列拒绝再次回填，避免并发旧响应把失效状态重新放回。只记录操作类型、账号 ID、模型和状态码，不输出状态值。淘汰后继续既有错误处理、退避及账号切换；不会清除账号额度或冷却记录。
...
OpenAI OAuth / SetupToken 的瞬时 `429` 同账号最多额外重试 3 次，即连首次发送最多 4 次，并同时受独立的 2 分钟重试截止时间约束。默认退避为 1、2、4 秒并增加 0–20% 正向抖动；上游 `Retry-After` 的秒数或 HTTP 日期要求更久时，完整保留该等待，不能缩短至固定秒数或剩余预算。剩余预算不足以容纳完整等待时停止同账号重试，进入既有账号切换或错误处理。

通用账号限流写入只允许延长冷却，迟到的短冷却不能覆盖已有较长冷却；显式管理恢复仍有独立入口。这些修复自动生效，不增加严格 RPM 或自适应并发控制。
