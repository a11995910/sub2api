...
OpenAI OAuth 账号编辑页只保留“异常时替换健康状态头（实验）”，对应 `extra.openai_healthy_turn_state_replace`，默认关闭。开启后使用已保存的动态 IP 接口、勾选模型及目标库存维护健康头；普通业务响应不再自动记录健康头。旧 `extra.openai_healthy_turn_state_record` 不再生效，创建、更新、额外字段更新和批量更新校验时会移除该旧字段；替换开关必须为布尔值，否则返回 `400 INVALID_HEALTHY_TURN_STATE_SETTING`。
...
自动维护由服务端负责，关闭弹窗、切换账号和浏览器离线均不影响运行，服务重启后从保存的配置恢复。扫描间隔为 15 秒，同一进程最多同时维护 4 个账号；只处理有效、已开启替换且保存了提取接口和勾选模型的 OpenAI OAuth 类账号。旧配置没有勾选模型时不自动发送请求。每次继续采集前重新检查开关、账号和配置；关闭替换后停止后续补位。连续健康采集间隔至少 1 秒，失败按 30 秒起步指数退避，最长 10 分钟。一次补位运行还受最多尝试入口数限制，默认 100，范围 1–1000，不能低于目标库存。失败、无头、已有同值头不增加有效库存；同一值重复返回不延长原有效期。

采集仅使用动态 IP 接口返回的临时代理，不使用直连或账号业务代理作为采集回退。保存账号自己的 HTTPS 提取地址和代理协议后，服务端按批提取代理并逐个发送一次固定的 `hi`。只有 5 秒内收到有效文本、推理或工具参数输出，且响应完整成功并包含有效 `X-Codex-Turn-State` 时才保存；单次请求最长 60 秒。HTTP 200、WS 握手成功、心跳、`response.created` 和空 delta 不算有效输出；429、503、模型不一致、空响应、断流、首字超时、无状态头或取消均不新增记录，也不会通过替换或普通请求重试掩盖失败。采集可在账号或模型冷却期间发送，不占用业务账号并发槽，不执行普通账号测试的状态恢复。
...
完整提取地址加密保存到设置表中的账号专属键，复用 `TOTP_ENCRYPTION_KEY`；管理接口只返回脱敏地址，输入留空保留已保存值。编辑页通过底部“更新”先保存采集配置，再保存账号及替换开关；采集配置使用独立管理接口存储，不修改业务代理。提取请求不使用账号代理或环境代理，最长 20 秒、响应最多 64 KiB、不跟随重定向；拨号拒绝内网、回环和元数据地址。临时代理不写入代理表、不回退直连，HTTP／WebSocket 客户端在单次采集后释放，不进入正式请求的共享代理缓存。提取失败保留明确错误，例如 HTTP 403 或白名单缺失；未知供应商正文不直接回显。连续三批没有新入口时停止本轮采集。

动态采集管理接口均位于 `/api/v1/admin/accounts/:id/healthy-turn-state/dynamic`，沿用管理员权限：

| 方法与后缀 | 请求与行为 |
| --- | --- |
| `GET /config` | 返回 `configured`、`api_url_masked`、`protocol`、`target_count`、`max_attempts`、`models`、`transport` |
| `PUT /config` | 保存 `api_url`（空值保留）、`protocol`（`http`／`https`／`socks5h`）、`target_count`、`max_attempts`、勾选的 `models`、`transport`（`http`／`websocket`） |
