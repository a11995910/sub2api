...
- `auto`：默认值；`base_url` 的 hostname 精确等于 `ai.cangyuansuanli.cn` 或 `vip-api.cangyuansuanli.cn` 时使用统一 JSON，精确等于 `api.zyca.top` 或 `canvas.zyca.top` 时使用 ZYCA，其他主机保持历史协议。
- `unified_json`：显式使用统一 JSON，适用于沧元自定义域名或其代理网关。
- `legacy`：强制使用现有 `seconds`、`image_urls` 历史协议。
- `zyca`：使用 ZYCA `/v1/generations`；自定义上游域名也可以显式选择。OpenAI API Key 账号的新增和编辑表单均提供“视频协议”选项。
...
视频计费独立保留 `768p/2K/4K` 档位，不折算到 480p。ZYCA 所有模型、所有分辨率均必须配置明确的分组视频价或渠道每秒价（支持对应分档或显式默认每秒价），否则提交前返回错误，包括订阅与模型测试路径。显式零价有效，不回退图片价格或其他供应商的系统默认价。管理员可在视频价格层级中填写上述档位，无需新增数据库列。价格校验与费用预留统一使用客户端原始请求模型名；渠道和账号模型映射只转换上游请求模型。

上游视频成本在“渠道 → 自定义账号统计定价规则”中手动维护，选择对应账号或分组，再按最终发往上游的模型名配置 `video`（每秒）定价。规则按顺序匹配，账号和分组为“或”关系，两者均为空时不匹配；模型先精确匹配，再匹配通配符。分辨率层级的 `per_request_price` 在 `video` 模式下表示每秒成本，未命中层级时仅允许使用该条目的显式默认价，零价有效。配置支持 `480p/720p/768p/1080p/2K/4K`；命中规则但缺少当前档位和默认价时不会借用其他规则、图片价格或客户售价。

账号统计成本为“配置的每秒成本 × 视频时长 × 视频数量 × 账号成本倍率”，用户实际扣费使用独立的客户定价、分组倍率和折扣。若成本配置已经包含上游账号折扣，账号成本倍率应设为 `1`，避免重复折扣。币种沿用现有渠道定价语义：`price_currency` 标记录入单位，系统不自动换汇，数值直接参与灵石账本；成本和售价须采用一致的核算单位后才能比较利润。成本来自人工配置，不代表已取得 ZYCA 的逐笔实际账单；上游倍率探测也不会同步这张模型成本表。

余额模式的异步视频在预留前保存成本基数和账号倍率快照到现有 `usage_context_json`，成功结算时写入 `account_stats_cost`、`account_rate_multiplier`，复用现有账号成本与利润统计，无需数据库迁移。ZYCA 任务缺少匹配的视频成本价时在调用上游前拒绝。提交后调整价格、倍率或规则不影响该任务；失败释放预留，不记成功用量；未知状态保留待核查。订阅和模型测试沿用原有非余额预留流程：按秒/按次计费的视频在提交前校验并保存请求内的售价、成本和账号倍率快照，用量记录直接使用快照，不再按渠道映射后的模型重新报价；其他协议中原有按 token 计费的视频保留原模型选择规则，按上游实际 token 用量结算，不生成视频固定报价或成本快照。ZYCA 仍须通过明确的每秒售价和成本校验。这份请求内快照不作为后台任务恢复记录持久化。
...
ZYCA 查询严格校验 `success`、任务 ID 和状态；未知响应保留任务待核查，不视为完成。下载每次先重新查询原任务获取 HTTPS 结果地址，复用已有公网地址、重定向、Range、媒体类型和大小校验，下载请求不携带上游 Key。后台恢复和计费沿用原任务记录，失败释放预留费用，明确完成且结果地址通过检查后才进入结算。任务执行期间应保持原账号的协议和上游地址不变；当前任务记录没有保存这两项配置的独立快照。
...
- Media targets for Grok groups: `/v1/images/generations`, `/images/generations`, `/v1/images/edits`, `/images/edits`, `/v1/videos/generations`, `/videos/generations`, `/v1/videos/edits`, `/videos/edits`, `/v1/videos/extensions`, `/videos/extensions`, `/v1/videos/{request_id}`, `/videos/{request_id}`, `/v1/videos/{request_id}/content`, and `/videos/{request_id}/content`. Generation, editing, and extension requests require the group image-generation permission. Video content endpoints resolve the bound task's xAI CDN URL, validate that it points to MP4 content, and proxy it through the same account.
