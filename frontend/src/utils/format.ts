 ...
 * 格式化货币金额
 * @param amount 金额
 * @returns 格式化后的字符串，如 "1.25 灵石"
 */
export function formatCurrency(amount: number | null | undefined): string {
  return formatSpiritStones(amount)
}

/**
 * 格式化平台统一余额单位：灵石。
 * 保留原数值精度策略，小额消耗展示 6 位，常规余额展示 2 位。
 */
export function formatSpiritStones(
  amount: number | null | undefined,
  options?: { fractionDigits?: number; suffix?: string }
): string {
  const value = Number.isFinite(Number(amount)) ? Number(amount) : 0
  const fractionDigits = options?.fractionDigits ?? (value > 0 && value < 0.01 ? 6 : 2)
  const locale = getLocale()
