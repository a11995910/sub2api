...
import { formatUSDScaled } from '@/utils/pricing'
...
function paidPerMillion(value: number | null | undefined): string {
  ...
  return formatUSDScaled(value * effectiveRate.value, PER_MILLION, MIN_DECIMALS)
}

/** 按次 / 按图片单价(乘生效倍率,不换算 1M)。 */
function paidRequestPrice(value: number | null | undefined): string {
  if (value == null) return '-'
  return formatUSDScaled(value * effectiveRate.value, 1, MIN_DECIMALS)
}

/** 官方参考价不乘倍率。 */
function official(value: number | null | undefined): string {
  if (value == null) return '-'
  return formatUSDScaled(value, PER_MILLION, MIN_DECIMALS)
}
