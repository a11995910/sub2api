    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <template v-else>
          ...
          />
          <UsageProgressBar
            v-if="!grokWeeklyBillingBar && !grokMonthlyBillingBar && grokRequestQuotaBar"
          ...
          />
          <UsageProgressBar
            v-if="!grokWeeklyBillingBar && !grokMonthlyBillingBar && grokTokenQuotaBar"
...
const loadUsage = async (options?: { source?: 'passive' | 'active'; bypassCache?: boolean }) => {
  ...
  try {
    const fetchFn = () => options?.source || options?.bypassCache
      ? adminAPI.accounts.getUsage(props.account.id, options?.source, options?.bypassCache)
...
const formatKeyCost = computed(() => {
  return formatSpiritStones(props.todayStats?.cost ?? 0)
})

const formatKeyUserCost = computed(() => {
  return formatSpiritStones(props.todayStats?.user_cost ?? 0)
})
...
watch(openAIUsageRefreshKey, (nextKey, prevKey) => {
  ...
  loadUsage({ bypassCache: true }).catch((e) => {
    console.error('Failed to refresh usage after account refresh key change:', e)
  })
})
