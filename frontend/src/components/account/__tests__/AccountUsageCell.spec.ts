...
describe('AccountUsageCell', () => {
  ...
  it('Key 账号会展示 today stats 徽章并带 A/U 提示', async () => {
		...
		expect(wrapper.text()).toContain('1.0B')
			expect(wrapper.text()).toContain('A 12.35 灵石')
			expect(wrapper.text()).toContain('U 6.79 灵石')
  ...
  it('Grok OAuth 保留本地用量与配额 fallback，同时兼容新版额度展示', async () => {
    getUsage.mockResolvedValue({
      grok_local_usage: {
        requests: 4,
        tokens: 1200,
  ...
  it('Vertex 账号会在 Gemini 用量窗口里展示 today stats 徽章', async () => {
		...
		expect(wrapper.text()).toContain('0')
			expect(wrapper.text()).toContain('A 0.00 灵石')
			expect(wrapper.text()).toContain('U 0.00 灵石')
  })
