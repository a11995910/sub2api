...
const trendChartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        label: `${t('usage.accountBilled')} (${t('common.currencyName')})`,
      ...
      {
        label: `${t('usage.userBilled')} (${t('common.currencyName')})`,
...
const lineChartOptions = computed(() => ({
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          if (label.includes(t('common.currencyName'))) {
  ...
  scales: {
    ...
    y: {
      ...
      title: {
        display: true,
        text: `${t('usage.accountBilled')} (${t('common.currencyName')})`,
        color: '#3b82f6',
        font: {
          size: 11
        }
...
const formatCost = (value: number): string => {
  if (value >= 1000) {
    return `${(value / 1000).toFixed(2)}K ${t('common.currencyName')}`
  }
  if (value >= 1) {
    return formatSpiritStones(value)
  }
  if (value >= 0.01) {
    return formatSpiritStones(value, { fractionDigits: 3 })
  }
  return formatSpiritStones(value, { fractionDigits: 4 })
}
