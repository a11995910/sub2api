...
describe("SubscriptionPlanCard", () => {
  ...
  it("uses spirit stones while preserving the configured currency label", () => {
    const cnyPlan = mountPlanCard("openai", { currency: "CNY", original_price: 20 }).text();

    expect(cnyPlan).toContain("10.00 灵石CNY");
    expect(cnyPlan).toContain("20.00 灵石");
    expect(mountPlanCard("openai", { currency: "USD" }).text()).toContain("10.00 灵石USD");
    expect(mountPlanCard("openai", { currency: "" }).text()).toContain("10.00 灵石");
  });
});
