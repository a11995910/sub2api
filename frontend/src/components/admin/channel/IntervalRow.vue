  ...
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <template v-if="mode === 'token'">
      <div class="pricing-interval-grid grid min-w-0 flex-1 gap-2">
        ...
        <div>
           <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice', '输入') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ tokenPriceUnit }}</span></label>
        ...
        <div>
           <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice', '输出') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ tokenPriceUnit }}</span></label>
          <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWrite5mPriceShort') }} <span class="text-gray-300">{{ tokenPriceUnit }}</span></label>
          <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWrite1hPriceShort') }} <span class="text-gray-300">{{ tokenPriceUnit }}</span></label>
          <input :value="interval.cache_write_1h_price" @input="emitField('cache_write_1h_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-gray-300">{{ tokenPriceUnit }}</span></label>
    ...
    <template v-else>
      <div class="w-24">
        ...
        <input :value="interval.tier_label" @input="emitField('tier_label', ($event.target as HTMLInputElement).value)"
          type="text" class="input mt-0.5 text-xs" :placeholder="mode === 'image' ? '1K / 2K / 4K' : mode === 'video' ? '480p / 720p / 768p / 1080p / 2K / 4K' : ''" />
      </div>
      <div class="w-20">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.minTokens') }}</label>
        <input :value="interval.min_tokens" @input="emitField('min_tokens', toInt(($event.target as HTMLInputElement).value))"
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ mode === 'video' ? t('admin.channels.form.perSecondPrice') : t('admin.channels.form.perRequestPrice', '单次价格') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ mode === 'video' ? perSecondPriceUnit : requestPriceUnit }}</span></label>
...
const requestPriceUnit = computed(() => props.currencySymbol ?? '灵石')
const tokenPriceUnit = computed(() => `${requestPriceUnit.value}/M`)
const perSecondPriceUnit = computed(() =>
  props.currencySymbol ? `${props.currencySymbol}/秒` : t('admin.channels.form.perSecondUnit'),
)
