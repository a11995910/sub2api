...
vi.mock('@/api/admin', () => ({ adminAPI: { groups: { getAllIncludingInactive: mocks.list }, users: { update: mocks.update } } }))
vi.mock('@/stores/app', () => ({ useAppStore: () => ({ showSuccess: vi.fn(), showError: vi.fn() }) }))
vi.mock('vue-i18n', () => ({ useI18n: () => ({ t: (key: string) => key }) }))
enableAutoUnmount(afterEach)
afterEach(() => vi.restoreAllMocks())
const response = [{ id: 7, name: 'Exclusive', platform: 'openai', is_exclusive: true, subscription_type: 'standard', status: 'active', rate_multiplier: 1 }]
beforeEach(() => {
  vi.clearAllMocks()
...
describe('UserAllowedGroupsModal load readiness', () => {
  ...
  it('preserves existing grants and rates on a successful save', async () => {
    ...
    await flushPromises()
    expect(mocks.update).toHaveBeenCalledWith(1, expect.objectContaining({ allowed_groups: [7], blocked_groups: [], restrict_public_groups: false, group_rates: { 7: 0.5 } }))
