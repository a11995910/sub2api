    ...
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      ...
      <div>
        ...
        <div class="relative flex gap-2">
          <div class="relative flex-1"><div class="absolute right-3 top-1/2 -translate-y-1/2 font-medium text-gray-500">{{ t('common.currencyName') }}</div><input v-model.number="form.amount" type="number" step="any" min="0" required class="input pr-14" /></div>
...
const formatBalance = (value: number) => formatSpiritStones(value)
