      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-orange-500">{{ t('usage.accountCost') }} {{ formatActualCost(totalAccountCost) }}</span>
            <span> · </span>
          </template>
          <span>
            {{ t('usage.standardCost') }}
            <span :class="{ 'line-through': strikeStandardCost }">{{ formatOriginalCost(stats?.total_cost || 0) }}</span>
          </span>
        </p>
      </div>
    </div>
