      ...
      >
        ...
        <div class="p-3">
          ...
          <div v-else class="space-y-2 text-gray-700 dark:text-gray-300">
            ...
            <div
              v-for="rate in effectiveRates"
              :key="rate.key"
              class="space-y-1.5 rounded-md border p-2"
              :class="[popoverBorderClass]"
            >
              <div class="flex items-center justify-between gap-2">
                ...
                <span
                  class="shrink-0 rounded bg-black/5 px-1.5 py-0.5 font-mono text-[10px] text-gray-600 dark:bg-white/10 dark:text-gray-300"
                >
                  {{ formatRate(rate.multiplier) }}x
                </span>
              </div>
...
import type { UserAvailableGroup, UserPricingInterval, UserSupportedModel, UserSupportedModelPricing } from '@/api/channels'
...
const effectiveRates = computed(() => {
  if (!props.groups || props.groups.length === 0) {
    return [{ key: 'default', name: t(prefixKey('defaultRate')), multiplier: 1 }]
  }

  return props.groups.map((group) => {
    const textRate = props.userGroupRates[group.id] ?? group.rate_multiplier ?? 1
    const imageRate = group.image_rate_independent
      ? group.image_rate_multiplier
      : textRate
    const multiplier = props.model.pricing?.billing_mode === BILLING_MODE_IMAGE
      ? imageRate
      : textRate
    return {
      key: String(group.id),
...
const primaryMultiplier = computed(() => effectiveRates.value[0]?.multiplier ?? 1)
...
function formatInterval(iv: UserPricingInterval, pricing: UserSupportedModelPricing): string {
  return formatIntervalWithMultiplier(iv, pricing, primaryMultiplier.value)
}
...
function formatIntervalWithMultiplier(iv: UserPricingInterval, pricing: UserSupportedModelPricing, multiplier: number): string {
  ...
  const input = formatScaled(resolved.input_price == null ? null : resolved.input_price * multiplier, perMillionScale)
  const output = formatScaled(resolved.output_price == null ? null : resolved.output_price * multiplier, perMillionScale)
  return `${input} / ${output}`
}
