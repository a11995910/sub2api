...
function formatInterval(iv: UserPricingInterval, mode: BillingMode): string {
  return formatIntervalWithMultiplier(iv, mode, primaryMultiplier.value)
}

function formatIntervalWithMultiplier(iv: UserPricingInterval, mode: BillingMode, multiplier: number): string {
  ...
  const input = formatScaled(iv.input_price == null ? null : iv.input_price * multiplier, perMillionScale)
  const output = formatScaled(iv.output_price == null ? null : iv.output_price * multiplier, perMillionScale)
  return `${input} / ${output}`
}
