  ...
  it.each(['availableChannels.pricing', 'admin.availableChannels.pricing'])(
    ...
    async (pricingKeyPrefix) => {
      ...
      try {
        ...
        expect(tooltip?.textContent).toContain(`0.05 USD ${pricingKeyPrefix}.unitPerSecond`)
        expect(tooltip?.textContent).toContain('480p')
        expect(tooltip?.textContent).toContain(`0 USD ${pricingKeyPrefix}.unitPerSecond`)
        expect(tooltip?.textContent).toContain('720p')
        expect(tooltip?.textContent).toContain(`0.12 USD ${pricingKeyPrefix}.unitPerSecond`)
        ...
        expect(tooltip?.textContent).toContain(`${pricingKeyPrefix}.videoPrice`)
        expect(wrapper.findComponent({ name: 'PricingRow' }).text()).toContain(`0 USD ${pricingKeyPrefix}.unitPerSecond`)
        await wrapper.setProps({
          model,
          groups: [{ id: 1, name: '用户分组', rate_multiplier: 2 } as UserAvailableGroup],
          userGroupRates: { 1: 0.2 }
        })
        expect(tooltip?.textContent).toContain(`0.01 USD ${pricingKeyPrefix}.unitPerSecond`)
        expect(tooltip?.textContent).toContain(`0.024 USD ${pricingKeyPrefix}.unitPerSecond`)
      } finally {
        wrapper.unmount()
      }
    }
