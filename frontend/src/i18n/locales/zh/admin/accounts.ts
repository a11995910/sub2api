export default {
    accounts: {
      ...
      openai: {
        ...
        requestIntegrityMode: '请求完整性观察',
        requestIntegrityModeDesc: '默认观察 Responses 转发中的上下文、推理参数和工具变化，仅记录变化字段，不保存对话正文，不拦截请求。可与指纹收敛同时使用。',
        requestIntegrityObserve: '观察（默认）',
        requestIntegrityOff: '关闭',
        healthyTurnStateRecord: '记录健康状态头',
        healthyTurnStateRecordDesc: '默认开启。完整健康响应且首字在 5 秒内返回的状态头会加密持久化，按账号和模型独立保存，重启后保留，最长有效 40 分钟。同账号同模型可在不同传输方式和代理出口间使用。关闭后停止自动新增，已有记录等待过期。',
        healthyTurnStateReplace: '异常时替换健康状态头（实验）',
        healthyTurnStateReplaceDesc: '默认关闭。HTTP 请求或 WebSocket 握手遇到 429／503，或响应模型与请求模型不一致时，从当前账号的同模型记录中取一条补试一次，仍失败则淘汰。不同传输方式和代理出口可共用同账号同模型的记录。模型不一致仅在尚未转发响应且可安全重放时补试。遵守上游等待时间；效果需实测，不能保证解除限流。',
        stateStats: {
          title: '健康状态头累计记录', refresh: '刷新统计',
          hint: '仅统计当前账号，并按模型分别累计；记录过期不影响累计数。无法归属账号的旧全局记录不计入。',
          error: '统计读取失败，请点击刷新重试。', empty: '暂无记录。自动记录默认开启，等待健康响应后会显示各模型累计数，也可手动采集。',
          accountCaptures: '当前账号累计', modelCaptures: '各模型累计记录', model: '模型', captures: '累计记录'
        },
        testTitle: '手动获取健康状态头',
        testConfigure: '选择代理与模型',
        testDesc: '使用账号已保存的配置，逐个出口发送一次 hi，模型一致、正常完成、有有效输出、状态头且首字在 5 秒内才记录。失败或 429／503 不记录、不重试。手动采集独立于自动记录开关，不修改账号代理；成功记录仅供当前账号和同一模型使用，可跨传输方式和代理出口。每项最长 60 秒。',
        testModel: '测试模型',
        testModelPlaceholder: '留空使用 gpt-6-astra',
        testTransport: '传输方式',
        testProxies: '测试出口（最多选择 5 个，依次测试）',
        testProxySearch: '搜索代理名称或地址',
        testNoProxies: '没有匹配的可用代理',
        testCurrentProxy: '账号当前保存的代理',
        testDirect: '直连（不使用代理）',
        testStart: '发送 hi 并记录',
        testRunning: '正在测试…',
        testResults: '状态头测试结果',
        testRequestFailed: '测试请求失败，请稍后重试',
        testExpires: '记录过期时间：{time}',
        dynamic: {
          mode: '采集方式', fixed: '固定代理', dynamic: '动态 IP 接口',
          description: '为当前账号单独保存提取接口。服务器读取接口返回的换行 IP:PORT，每次使用一个代理入口采集；URL 中 num 控制每批数量，time 等供应商参数原样传递。代理入口不一定等于实际出口 IP，请按供应商支持选择协议。仅新增记录计入目标，达到目标或尝试上限即停止；关闭页面会停止后续采集，不在后台续跑。',
          url: '代理提取 URL', urlPlaceholder: '输入提取接口；留空保留已保存地址', savedUrl: '已保存：{url}', protocol: '代理协议（可选）',
          target: '目标新增数', maxAttempts: '最多尝试入口数',
          validation: '请填写提取 URL；目标为 1–100，最多尝试为 1–1000 且不能小于目标。',
          save: '保存配置', saving: '正在保存…', saved: '配置已保存', reload: '重新加载', start: '保存并开始采集', starting: '正在保存并启动…', stopping: '已取消，正在确认停止…',
          loadFailed: '动态 IP 配置读取失败，请重试。', saveFailed: '配置保存失败，请检查提取 URL 和数量后重试。', runFailed: '动态采集失败，已停止后续尝试。请检查提取接口和代理协议后重试。', stopFailed: '停止状态确认失败，页面已停止发送后续采集请求。',
          progress: '新增 {recorded} / {target} · 已尝试 {attempts} / {max} · 已提取 {batches} 批', attempt: '第 {count} 次尝试',
          status: { running: '正在采集', completed: '采集已结束', stopped: '采集已停止', failed: '采集失败' }
        },
        testStatus: {
          pending: '等待测试',
          running: '正在测试',
          recorded: '已记录',
          already_recorded: '已有相同有效记录',
          no_header: '响应正常，未返回状态头',
          unhealthy: '响应不健康或模型不一致，未记录',
          not_recorded: '记录不可用、正在使用或保存失败，未新增',
          failed: '测试失败，未记录',
          cancelled: '已取消'
        },
