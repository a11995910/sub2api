export default {
    accounts: {
      ...
      openai: {
        ...
        healthyTurnStateReplaceDesc: '默认关闭。HTTP 请求或 WebSocket 握手遇到 429／503 时，从 OpenAI 全局共享池取一条记录补试一次，不按账号、模型、传输方式或代理出口隔离，仍失败则淘汰。遵守上游等待时间；效果需实测，不能保证解除限流。',
        stateStats: {
          title: '健康状态头记录与效果', refresh: '刷新统计',
          hint: '记录加密持久化，重启保留。所有 OpenAI 账号共用同一个池，不按模型、传输方式或代理出口隔离；未过期表示可尝试。只有替换后收到有效输出并完整结束，才计为调用成功。',
        ...
        testDesc: '使用账号已保存的配置，逐个出口发送一次 hi，正常完成、有有效输出、状态头且首字在 5 秒内才记录。失败或 429／503 不记录、不重试。手动采集独立于自动记录开关，不修改账号代理；成功记录进入 OpenAI 全局共享池，可供其他账号、模型、出口和传输方式使用。每项最长 60 秒。',
        testModel: '测试模型',
        testModelPlaceholder: '留空使用 gpt-5.6-sol',
        testTransport: '传输方式',
