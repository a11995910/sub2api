export default {
    accounts: {
      ...
      openai: {
        ...
        healthyTurnStateRecordDesc: '默认关闭。开启后，完整健康响应且首字在 5 秒内返回的状态头会加密持久化，最长有效 40 分钟，进入 OpenAI 全局共享池并在重启后保留。关闭后停止自动新增，已有记录等待过期。',
        healthyTurnStateReplace: '异常时替换健康状态头（实验）',
        healthyTurnStateReplaceDesc: '默认关闭。HTTP 请求或 WebSocket 握手遇到 429／503，或响应模型与请求模型不一致时，从 OpenAI 全局共享池取一条记录补试一次，不按账号、模型、传输方式或代理出口隔离，仍失败则淘汰。模型不一致仅在尚未转发响应且可安全重放时补试。遵守上游等待时间；效果需实测，不能保证解除限流。',
        stateStats: {
          title: '健康状态头记录与效果', refresh: '刷新统计',
          hint: '记录加密持久化，重启保留。所有 OpenAI 账号共用同一个池，不按模型、传输方式或代理出口隔离；未过期表示可尝试。只有替换后模型一致、收到有效输出并完整结束，才计为调用成功。',
          error: '统计读取失败，请点击刷新重试。', empty: '暂无记录（共享池为空）。可手动采集，或开启自动记录后等待健康响应。',
          available: '可尝试记录', captures: '累计记录', successFailure: '替换成功 / 失败', successRate: '替换成功率',
          attempts: '累计发送 {count} 次替换，当前 {busy} 条占用中；未结束或中断且无结果的调用不计入成功率。',
          records: '查看记录与最近使用结果（最多 50 项）', probes: '最近手动采集结果（最多 50 项）', proxy: '代理 #{id}',
          results: '替换成功 {success} 次，失败 {failed} 次', capturedAt: '最近采集：{time}', expiresAt: '有效期至：{time}', usedAt: '最近使用：{time}',
          state: { available: '未过期，可尝试', in_use: '占用中', expired: '已过期', invalid: '不可用' },
          outcome: { pending: '替换结果尚未确认', success: '最近替换调用成功', failed: '最近替换调用失败', decrypt_failed: '记录解密失败，已停用' }
        },
        testTitle: '手动获取健康状态头',
        testConfigure: '选择代理与模型',
        testDesc: '使用账号已保存的配置，逐个出口发送一次 hi，模型一致、正常完成、有有效输出、状态头且首字在 5 秒内才记录。失败或 429／503 不记录、不重试。手动采集独立于自动记录开关，不修改账号代理；成功记录进入 OpenAI 全局共享池，可供其他账号、模型、出口和传输方式使用。每项最长 60 秒。',
        testModel: '测试模型',
