export default {
    accounts: {
      ...
      openai: {
        ...
        healthyTurnStateRecordDesc: '默认关闭。开启后，完整健康响应返回的状态头加密持久化保存，最长有效 40 分钟，重启保留。关闭后停止自动新增，已有记录等待过期。',
        healthyTurnStateReplace: '异常时替换健康状态头（实验）',
        healthyTurnStateReplaceDesc: '默认关闭。HTTP 请求或 WebSocket 握手遇到 429／503 时，用同账号、同模型和传输方式的记录补试一次，仍失败则淘汰。遵守上游等待时间；效果需实测，不能保证解除限流。',
        stateStats: {
          title: '健康状态头记录与效果', refresh: '刷新统计',
          hint: '记录加密持久化，重启保留。未过期表示可尝试；只有替换后收到有效输出并完整结束，才计为调用成功。成功率按已结束的替换计算。',
          error: '统计读取失败，请点击刷新重试。', empty: '暂无记录。可手动采集，或开启自动记录后等待健康响应。',
        ...
        testDesc: '使用账号已保存的配置，逐个出口发送一次 hi，正常完成且有有效输出和状态头才记录。失败或 429／503 不记录、不重试。手动采集独立于自动记录开关，不修改账号代理；记录只供同账号、同模型、同出口和同传输方式使用。每项最长 60 秒。',
        testModel: '测试模型',
