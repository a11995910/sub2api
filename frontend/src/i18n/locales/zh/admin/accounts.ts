export default {
    accounts: {
      ...
      openai: {
        ...
        codexTicketResults: {
          success: '采集成功', invalid_format: '未取得有效 292／332 门票', auth_failed: '认证失败',
        ...
        dynamic: {
          ...
          url: '全局代理提取 URL', urlPlaceholder: '首次配置时填写；留空保留全局地址', savedUrl: '已配置全局接口：{url}。当前账号直接复用，无需重复填写；留空保持不变，填写新地址会更新所有账号。', urlRequired: '全局填写一次，其他账号直接复用。支持 HTTP／HTTPS 接口，返回逐行 IP:PORT 或 BestGo JSON；count、num、time 等供应商参数原样传递。', protocol: '全局代理协议',
          sharedProxyConflict: '已有账号保存了不同接口，请填写一次统一的全局地址。',
          target: '每个模型的健康头数量', targetHint: '默认 2 个，范围 1–100；已保存账号保留原数量。使用中的健康头仍计入目标，剩余不足 10 分钟的空闲头提前补新。', targetPreset: '设为每模型 {count} 个', targetSummary: '已选 {models} 项，预计最多 {total} 个；相同实际模型共用库存。', maxAttempts: '每轮最多尝试入口数',
