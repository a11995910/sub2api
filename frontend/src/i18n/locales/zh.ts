  ...
  admin: {
    ...
    groups: {
      ...
      imagePricing: {
        ...
        image2KEnhancementHint: '当前分组命中显式 2K 生图时，先生成原图，再用目标图片分组按原始 size 做二段超分。',
        image2KEnhancementGroup: '2K 超分目标分组',
        selectImage2KEnhancementGroup: '选择图片分组',
        noImage2KEnhancementGroup: '不使用 2K 超分分组',
        image2KEnhancementGroupHint: '目标分组需为 OpenAI 图片分组；请求中的 size 会原样传入二段超分，目标失败 3 次后返回原图。',
        image2KEnhancementGroupRequired: '开启 2K 超分时必须选择目标图片分组',
        ...
        image4KEnhancementGroupHint: '目标分组需为 OpenAI 图片分组；请求中的 size 会原样传入二段提升，目标失败 3 次后返回原图。',
    ...
    settings: {
      ...
      site: {
        ...
        apiBaseUrl: 'API 端点地址',
        apiBaseUrlHint: '用于"使用密钥"和"导入到 CC Switch"功能；建议填写客户端本机可访问的 HTTPS 公网地址，留空则使用当前站点地址。修改后请重新导入 CCS。',
