export default {
  ...
  keys: {
    ...
    quotaLimit: '额度限制',
    quotaAmount: '额度金额（灵石）',
    quotaAmountPlaceholder: '输入灵石额度限制',
  ...
  oauthAccountPool: {
    ...
    overviewLabel: '号池概览',
    overviewDescription: '展示套餐、额度与实时运行状态，方便快速了解当前服务可用性。',
  ...
  usage: {
    ...
    cacheHitControlDetails: '缓存命中率控制',
    cacheHitOriginalInputTokens: '原始普通输入',
    cacheHitOriginalCacheReadTokens: '原始缓存读取',
    cacheHitShiftedTokens: '划入普通输入',
    cacheHitTarget: '目标与容差',
    cacheHitCumulative: '调整后累计命中率',
    cacheHitStateVersion: '状态代次',
  ...
  availableChannels: {
    ...
    public: '公开',
    imageEnabled: '图片可用',
    imageRateTooltip: '图片生成最终倍率：{rate}x',
  ...
  checkin: {
    title: '每日签到',
    description: '每天领取灵石，连续签到每 16 天循环获得额外奖励。',
    ...
    dailyReward: '每日奖励',
    day4Bonus: '连续第 4 天奖励',
    day16Bonus: '连续第 16 天奖励',
    nextBonus: '下一档额外奖励',
    daysToBonus: '还差 {days} 天到下一档（周期第 {milestone} 天）',
    noMoreBonus: '当前连续周期的额外奖励已全部领取',
    currentBalance: '当前余额：{balance}',
    monthCalendar: '{year} 年 {month} 月签到表',
    rulesTitle: '签到规则',
    ruleDaily: '每日只能签到一次，签到成功后发放 {reward}。',
    ruleBonus4: '每个连续签到周期第 4 天签到时，额外发放 {reward}。',
    ruleBonus16: '每个连续签到周期第 16 天签到时，额外发放 {reward}。',
    ruleMonthReset: '连续签到可跨自然月累计，每 16 天完成一轮后从第 1 天重新计算；任意一天未签到也从第 1 天重新计算。',
    loadFailed: '加载签到信息失败',
    failed: '签到失败',
  ...
  modelMarket: {
    ...
    officialReference: '渠道计费基准',
    discountCompared: '比渠道计费基准低 {value}',
    sort: {
      label: '模型排序',
  ...
  admin: {
    ...
    users: {
      ...
      // 用户分组配置
      groupConfig: '用户分组配置',
      groupConfigHint: '为用户 {email} 配置分组权限和专属倍率',
    ...
    groups: {
      ...
      deleteConfirm: "确定要删除分组 '{name}' 吗？绑定该分组的 API 密钥会迁移到下方选择的替换分组；不选择则使用 API Key 默认分组。",
      deleteConfirmSubscription:
        "确定要删除订阅分组 '{name}' 吗？此操作会删除相关订阅记录；绑定该分组的 API 密钥会迁移到下方选择的替换分组，不选择则使用 API Key 默认分组。此操作无法撤销。",
      deleteReplacementLabel: 'API Key 替换分组',
      deleteReplacementUseDefault: '不指定，使用 API Key 默认分组',
      deleteReplacementHint: '只迁移仍绑定当前分组的 API Key。替换分组必须是启用状态；不选择时继续使用系统设置里的 API Key 默认分组兜底。',
      deleteReplacementLoading: '正在加载可替换分组...',
      deleteReplacementLoadFailed: '加载可替换分组失败，将暂时使用当前列表候选',
      deleteReplacementNoCandidates: '当前没有其他启用分组可选，确认后将尝试使用 API Key 默认分组。',
      columns: {
        name: '名称',
        platform: '平台',
        rateMultiplier: '费率倍数',
      ...
      form: {
        ...
        cacheHitTargetEnabled: '控制用户缓存命中率',
        cacheHitTargetEnabledHint: '仅对成功终止的 OpenAI HTTP 流式 Chat Completions/Responses 请求生效；下游 usage 与本站计费共用同一次划拨结果。',
        cacheHitTargetPercent: '目标命中率',
        cacheHitTargetTolerancePercent: '容差带',
        cacheHitHalfLifeDays: '历史半衰期',
        daysUnit: '天',
        cacheHitTargetPercentHint: '默认目标 90%、容差 ±0.50%、历史半衰期 1 天；其他协议、非流式、WebSocket、compact、input_tokens、图片和视频请求均不生效。',
        cacheHitTargetPercentInvalid: '目标命中率必须在 0.01% 到 100% 之间',
        cacheHitTargetTolerancePercentInvalid: '容差必须在 0% 到 50% 之间，且目标加减容差必须保持在 0% 到 100% 之间',
        cacheHitHalfLifeDaysInvalid: '历史半衰期必须在 0.01 天到 365 天之间',
        exclusiveLabel: '专属分组',
      ...
      imagePricing: {
		...
		responseFormat: '默认传输方式',
		responseFormatBase64: 'Base64',
		responseFormatURL: 'URL（本地保存 24 小时）',
		responseFormatExplicitHint: '客户请求显式传入 response_format 时，以客户参数为准。',
    ...
    channels: {
      ...
      form: {
        ...
        addRule: '添加规则',
        noRulesConfigured: '未配置自定义账号成本规则。',
        ruleName: '规则名称（可选）',
        ruleGroups: '分组',
        ruleAccounts: '账号',
        searchAccountPlaceholder: '搜索账号...',
        ruleAccountsHint: '匹配所选账号或分组；两者都为空时不匹配。',
    ...
    riskControl: {
      ...
      unbanSuccess: '用户已解封',
      unbanFailed: '解封用户失败',
      inputDetailTitle: '输入详情',
      inputDetailContent: '原始输入（已脱敏）',
      inputDetailExcerpt: '已保存的输入摘要',
      inputDetailRedactedHint: '展示本次审核的最后一条用户输入文本，保留换行，敏感信息已脱敏。',
      inputDetailUnavailableHint: '此记录未保存原始输入（历史记录或未采集原文的事件），只能查看当时保存的摘要，无法还原完整输入。',
      inputDetailTruncatedHint: '脱敏后的输入共 {count} 个字符，仅保存前 65,536 个字符，以下内容已截断。',
      inputDetailImagesHint: '本次输入包含 {count} 张图片；仅记录图片数量，未保存图片地址或图片数据。',
      inputDetailNoText: '此记录没有可展示的输入文本。',
      inputDetailAuditError: '审核错误信息',
      inputDetailLoadFailed: '加载输入详情失败，请重试。',
    ...
    accounts: {
      ...
      dataActions: '数据操作',
      toolActions: '工具',
      checkAllCodexAccounts: '检查全部 Codex 账号',
      checkingAllCodexAccounts: '正在检查 Codex 账号...',
      checkAllCodexConfirm: '确认检查全部 OpenAI OAuth / Codex 账号令牌状态吗？失效账号会被标记为错误状态。',
      checkAllCodexEmpty: '暂无可检查的 Codex 账号',
      checkAllCodexSuccess: '已完成 Codex 账号检查，成功 {count} 个',
      checkAllCodexPartial: 'Codex 账号检查完成：成功 {success} 个，失败 {failed} 个',
      checkAllCodexFailed: 'Codex 账号检查失败',
      viewColumns: '列显示',
      ...
      dataImportHint: '支持批量上传 Sub2API 导出数据，或多个 Codex Agent Identity auth.json。',
      dataImportWarning: '两类文件不能混选。Agent Identity 按 ChatGPT 账号更新重复项，导入后需手工绑定分组。',
      dataImportFile: 'JSON 文件',
      dataImportButton: '开始导入',
      dataImporting: '导入中...',
      dataImportSelectFile: '请选择数据文件',
      dataImportParseFailed: '数据解析失败',
      dataImportParseFailedFile: '文件 {name} 解析失败',
      dataImportInvalidFile: '文件 {name} 不是受支持的 Sub2API 导出数据或 Agent Identity auth.json',
      dataImportMixedFileTypes: '不能同时导入 Sub2API 导出数据和 Agent Identity auth.json，请分两次操作',
      ...
      agentIdentityImportResultSummary: 'Agent Identity 新增 {created}，更新 {updated}，跳过 {skipped}，失败 {failed}',
      agentIdentityImportWarning: '警告',
      agentIdentityImportSuccess: 'Agent Identity 导入完成：新增 {created}，更新 {updated}，跳过 {skipped}',
      agentIdentityImportCompletedWithErrors: 'Agent Identity 部分导入失败：新增 {created}，更新 {updated}，跳过 {skipped}，失败 {failed}',
      ...
      quotaLimitHint: '设置日/周/总使用额度（灵石），任一维度达到限额后账号暂停调度。Anthropic API Key 账号还可配置客户端亲和。修改限额不会重置已用额度。',
      ...
      bulkEdit: {
        title: '批量编辑账号',
        filteredScope: '当前筛选结果',
        selectionInfo: '将更新已选中的 {count} 个账号。只更新您勾选或填写的字段，未勾选的字段保持不变。',
        filteredSelectionInfo: '将更新当前筛选结果中的 {count} 个账号（不限本页）。只更新您勾选或填写的字段，未勾选的字段保持不变。',
        baseUrlPlaceholder: 'https://api.anthropic.com 或 https://api.openai.com',
        baseUrlNotice: '仅适用于 API Key 账号，留空则不修改',
        submit: '批量更新',
        updateSelected: '更新已选账号',
        updateFiltered: '更新筛选结果',
        updating: '更新中...',
    ...
    settings: {
      ...
      features: {
        ...
        checkin: {
          ...
          dailyReward: '每日固定奖励',
          extraReward4: '连续第 4 天额外奖励',
          extraReward16: '连续第 16 天额外奖励',
          rewardHint: '奖励必须为非负数；启用签到时每日固定奖励必须大于 0。额外奖励按 16 天一个周期，在周期第 4 天、第 16 天发放，周期结束或断签后从第 1 天重新计算。',
      ...
      defaults: {
        ...
        apiKeyDefaultGroup: 'API Key 默认分组',
        apiKeyDefaultGroupHint: '删除已绑定 Key 的分组时，这些 Key 会自动迁移到该分组；留空则禁止删除仍有 Key 绑定的分组。',
        noApiKeyDefaultGroup: '不设置默认分组',
      ...
      site: {
        ...
        apiBaseUrl: 'API 端点地址',
        apiBaseUrlHint: '用于"使用密钥"、"导入到 CC Switch"和回调地址建议；建议填写客户端本机可访问的 HTTPS 公网地址，留空则使用当前站点地址。修改后请重新导入 CCS。',
        ...
        quickLinkTitle: '顶部快捷入口',
        quickLinkDescription: '在登录后的顶部栏展示一个醒目的外部链接，可用于客服、公告或社群入口',
        quickLinkText: '入口文案',
        quickLinkTextPlaceholder: '联系客服：点击进入 aibang.click，务必保存网址',
        quickLinkTextHint: '建议 8-40 个字，过长会在顶部栏自动截断',
        quickLinkUrl: '入口链接',
        quickLinkUrlPlaceholder: 'https://aibang.click',
        quickLinkUrlHint: '必须填写 http(s) 绝对链接，点击后会在新窗口打开',
        quickLinkTextRequired: '启用顶部快捷入口时必须填写入口文案',
        quickLinkUrlInvalid: '顶部快捷入口链接必须是有效的 http(s) 地址',
        siteLogo: '站点Logo',
        uploadImage: '上传图片',
        remove: '移除',
      ...
      payment: {
        ...
        balanceRechargeMultiplierHint: '用户每支付 1 CNY 可获得多少灵石余额',
        balanceRechargePreview: '预览：1 CNY = {usd} 灵石',
        subscriptionUsdToCnyRate: '订阅 CNY 换算汇率',
        subscriptionUsdToCnyRateHint:
          'CNY 支付通道下，套餐每 1 灵石价格收取多少 CNY（如 7.15）。0 或留空 = 不换算，订阅按 price 数值直接收款。启用后所有套餐 price 必须按灵石定价',
      ...
      openaiFastPolicy: {
        ...
        userIdsInvalid: '用户 ID 必须是大于 0 且不重复的安全整数。',
        errorMessage: '错误消息',
        errorMessagePlaceholder: '拦截时返回的自定义错误消息',
        errorMessageHint: '留空则使用默认错误消息。',
        modelWhitelist: '目标模型',
  ...
  onboarding: {
    ...
    admin: {
      ...
      groupMultiplier: {
        title: '💰 3. 费率倍数',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">设置该分组的计费倍率，控制用户的实际扣费。</p><div style="padding: 8px 12px; background: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>⚙️ 计费规则：</b><ul style="margin: 8px 0 0 16px;"><li><b>1.0</b> - 原价计费（成本价）</li><li><b>1.5</b> - 用户消耗 1 灵石，扣除 1.5 灵石</li><li><b>2.0</b> - 用户消耗 1 灵石，扣除 2 灵石</li><li><b>0.8</b> - 补贴模式（亏本运营）</li></ul></div><p style="font-size: 13px; color: #6b7280;">建议测试分组设置为 1.0</p></div>',
        nextBtn: '下一步'
      },
      groupExclusive: {
        title: '🔒 4. 专属分组（可选）',
  ...
  payment: {
    ...
    admin: {
      ...
      userBalance: '用户余额',
      orderAmount: '订单金额',
      insufficientBalance: '余额不足，将扣至 0 灵石',
