    ...
    settings: {
      ...
      features: {
        ...
        modelPlaza: {
          ...
          usdToCnyRate: 'Model Market USD to CNY rate',
          usdToCnyRateHint: 'Only converts USD channel base prices to CNY for Model Market discount display. It never changes actual stone billing.',
          usdToCnyRateInvalid: 'The USD to CNY rate must be between 0.01 and 100.',
      ...
      gatewayForwarding: {
        ...
        codexTicketEnabled: 'Upstream 292/332 ticket collection',
        codexTicketEnabledDesc: 'Applies to accounts using the upstream 292/332 ticket strategy. Collects tickets in the background and injects them into requests.',
        codexTicketModelMismatchInvalidation: '模型不一致（含路由到 Luna）弃票',
        codexTicketModelMismatchInvalidationDesc: '默认开启。采票或业务响应声明的模型与请求模型不一致时弃票。关闭后允许模型偏移，其他失效检查仍生效。',
        codexTicketUseHarvestProxy: '后续请求使用打票代理 IP',
        codexTicketUseHarvestProxyDesc: '默认开启。业务请求沿用成功采票的代理；关闭后使用账号原本的代理配置，未配置则直连。同一实际 IP 仍需代理供应商提供固定或粘性出口。',
        codexTicketHarvestMode: '292/332 ticket collection source',
        codexTicketHarvestModeProxy: 'Fixed proxy URL',
        codexTicketHarvestModeExtract: 'Batch IP extraction API',
        codexTicketHarvestModeHint: '成功门票与采集代理按账号、模型绑定，业务请求持续使用该代理，直到门票或出口失效。切换采集来源保留已有有效绑定。',
        codexTicketHarvestProxy: '292/332 ticket collection proxy',
        codexTicketHarvestProxyDesc: '填写完整 HTTP(S) 或 SOCKS5(h) 代理 URL。保持实际出口需使用固定或粘性 IP，并关闭供应商提前换 IP；粘性时长应覆盖门票期限。留空或保留已隐藏的密码表示不修改。',
