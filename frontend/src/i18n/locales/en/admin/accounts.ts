export default {
    accounts: {
      ...
      openai: {
        ...
        healthyTurnStateRecordDesc: 'Disabled by default. Encrypt and persist state headers from complete healthy responses for up to 40 minutes, surviving restarts. Disabling stops automatic collection; existing records expire normally.',
        healthyTurnStateReplace: 'Replace turn state on errors (experimental)',
        healthyTurnStateReplaceDesc: 'Off by default. On an HTTP or WebSocket handshake 429/503, retry once with a saved state from the same account, model and transport. Discard it if the retry fails. Respects upstream retry delays; effectiveness is unverified and rate limits may persist.',
        stateStats: {
          title: 'Saved state headers and results', refresh: 'Refresh',
          hint: 'Records are encrypted and survive restarts. An unexpired record can be tried; success requires meaningful output and a completed response after replacement. Success rate includes only completed attempts.',
          error: 'Statistics could not be loaded. Refresh to retry.', empty: 'No records yet. Collect manually or enable automatic recording and wait for a healthy response.',
        ...
        testDesc: 'Use the saved account settings to send hi once through each selected route. Record only a complete response with meaningful output and a state header. Errors, including 429/503, are not recorded or retried. Manual collection works independently of the automatic recording switch and does not change the account proxy. Records apply to the same account, model, route and transport. Each test takes up to 60 seconds.',
