export default {
    accounts: {
      ...
      openai: {
        ...
        requestIntegrityMode: 'Request integrity observation',
        requestIntegrityModeDesc: 'Observe changes to context, reasoning settings and tools during Responses forwarding by default. Logs field names only, without conversation content or blocking requests. Compatible with fingerprint convergence.',
        requestIntegrityObserve: 'Observe (default)',
        requestIntegrityOff: 'Off',
        healthyTurnStateRecord: 'Record healthy turn state',
        healthyTurnStateRecordDesc: 'Enabled by default. Encrypt and persist state headers from complete healthy responses whose first output arrives within 5 seconds, for up to 40 minutes, surviving restarts. Records are isolated by account and model, and shared across transports and proxies within that account and model. Disabling stops automatic collection; existing records expire normally.',
        healthyTurnStateReplace: 'Replace turn state on errors (experimental)',
        healthyTurnStateReplaceDesc: 'Off by default. On an HTTP or WebSocket handshake 429/503, or a response model mismatch, retry once with a saved state for the same account and model. Discard it if the retry fails. Records may be shared across transports and proxies within that account and model. Model mismatch retries require no response to have been forwarded and a safely replayable request. Respects upstream retry delays; effectiveness is unverified and rate limits may persist.',
        stateStats: {
          title: 'Cumulative healthy state records', refresh: 'Refresh',
          hint: 'Counts belong to this account and are grouped by model. Expired records remain in the totals. Legacy global records without an account owner are excluded.',
          error: 'Statistics could not be loaded. Refresh to retry.', empty: 'No records yet. Automatic recording is enabled by default; model totals appear after healthy responses. You can also collect manually.',
          accountCaptures: 'Account total', modelCaptures: 'Cumulative records by model', model: 'Model', captures: 'Total records'
        },
        testTitle: 'Collect healthy turn state manually',
        testConfigure: 'Choose proxies and model',
        testDesc: 'Use the saved account settings to send hi once through each selected route. Record only a model-matching, complete response with meaningful output, a state header and a first output within 5 seconds. Errors, including 429/503, are not recorded or retried. Manual collection works independently of the automatic recording switch and does not change the account proxy. Successful records belong only to this account and model, and can be shared across transports and proxies. Each test takes up to 60 seconds.',
        testModel: 'Test model',
        testModelPlaceholder: 'Leave blank to use gpt-6-astra',
        testTransport: 'Transport',
        testProxies: 'Test routes (up to 5, tested in sequence)',
        testProxySearch: 'Search proxy name or address',
        testNoProxies: 'No matching available proxies',
        testCurrentProxy: 'Currently saved account proxy',
        testDirect: 'Direct connection (no proxy)',
        testStart: 'Send hi and record',
        testRunning: 'Testing…',
        testResults: 'Turn state test results',
        testRequestFailed: 'Test request failed. Please try again later.',
        testExpires: 'Record expires at {time}',
        dynamic: {
          mode: 'Collection method', fixed: 'Fixed proxies', dynamic: 'Dynamic IP API',
          description: 'Save a separate extraction API for this account. The server reads newline-separated IP:PORT entries and tries one proxy entry at a time. The URL num parameter controls batch size; time and other provider parameters are passed through unchanged. A proxy entry may differ from the actual exit IP; choose a protocol supported by your provider. Only newly recorded states count toward the target. Collection stops at the target or attempt limit. Closing the page stops further collection; it does not continue in the background.',
          url: 'Proxy extraction URL', urlPlaceholder: 'Enter the API URL; leave blank to keep the saved URL', savedUrl: 'Saved: {url}', protocol: 'Proxy protocol (selectable)',
          target: 'New records target', maxAttempts: 'Maximum proxy attempts',
          validation: 'Provide an extraction URL. The target must be 1–100, and attempts must be 1–1000 and at least the target.',
          save: 'Save settings', saving: 'Saving…', saved: 'Settings saved', reload: 'Reload', start: 'Save and start collection', starting: 'Saving and starting…', stopping: 'Cancelled; confirming stop…',
          loadFailed: 'Could not load dynamic IP settings. Please retry.', saveFailed: 'Could not save settings. Check the extraction URL and limits, then retry.', runFailed: 'Dynamic collection failed and further attempts have stopped. Check the extraction API and proxy protocol, then retry.', stopFailed: 'Could not confirm the stop status. This page has stopped sending collection requests.',
          progress: 'New {recorded} / {target} · Attempts {attempts} / {max} · Batches {batches}', attempt: 'Attempt {count}',
          status: { running: 'Collecting', completed: 'Collection finished', stopped: 'Collection stopped', failed: 'Collection failed' }
        },
        testStatus: {
          pending: 'Waiting',
          running: 'Testing',
          recorded: 'Recorded',
          already_recorded: 'Identical valid record exists',
          no_header: 'Healthy response, no state header',
          unhealthy: 'Incomplete or empty response; not recorded',
          not_recorded: 'Record unavailable, in use, or storage failed; not added',
