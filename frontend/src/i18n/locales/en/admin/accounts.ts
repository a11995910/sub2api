export default {
    accounts: {
      ...
      openai: {
        ...
        turnStateMode: '状态头策略',
        turnStateModes: { off: '关闭', healthy_retry: '健康头异常补试', healthy_preflight: '健康头首发注入', codex_ticket: '上游 292 门票' },
        turnStateModeHints: {
        ...
        stateStats: {
          attempts: '使用', successes: '成功', failures: '失败',
          historyHint: '使用、成功和失败均为本账号各健康头模式的历史累计，不是当前模式的独立成功率；使用中请求可能尚未计入结果。',
          title: 'Healthy state inventory', refresh: 'Refresh',
          hint: '有效库存包含可用和使用中的健康头；剩余不足 10 分钟时提前补新，旧头仍可使用至 40 分钟本地期限。累计采集数包含历史记录。',
        ...
        testModel: 'Test models (select one or more)',
        ...
        dynamic: {
          ...
          url: 'Global proxy extraction URL', urlPlaceholder: 'Enter once; leave blank to keep the global URL', savedUrl: 'Global API configured: {url}. This account uses it automatically. Leave blank to keep it; a new URL updates all accounts.', urlRequired: 'Configure once for all accounts. Supports HTTP/HTTPS APIs returning newline-separated IP:PORT entries or BestGo JSON; provider parameters such as count, num and time are passed through unchanged.', protocol: 'Global proxy protocol',
