export default {
    accounts: {
      ...
      openai: {
        ...
        healthyTurnStateRecordDesc: 'Disabled by default. Encrypt and persist state headers from complete healthy responses whose first output arrives within 5 seconds, for up to 40 minutes, surviving restarts. Records enter the shared OpenAI pool. Disabling stops automatic collection; existing records expire normally.',
        healthyTurnStateReplace: 'Replace turn state on errors (experimental)',
        healthyTurnStateReplaceDesc: 'Off by default. On an HTTP or WebSocket handshake 429/503, or a response model mismatch, retry once with a saved state from the shared OpenAI pool, regardless of account, model, transport or proxy. Discard it if the retry fails. Model mismatch retries require no response to have been forwarded and a safely replayable request. Respects upstream retry delays; effectiveness is unverified and rate limits may persist.',
        stateStats: {
          title: 'Saved state headers and results', refresh: 'Refresh',
          hint: 'Records are encrypted and survive restarts. This is one shared OpenAI pool: any account can use an unexpired record, regardless of model, transport or proxy. Success requires meaningful output and a completed response after replacement.',
          error: 'Statistics could not be loaded. Refresh to retry.', empty: 'No shared records yet. Collect manually or enable automatic recording and wait for a healthy response.',
          available: 'Ready to try', captures: 'Total records', successFailure: 'Succeeded / Failed', successRate: 'Success rate',
          attempts: '{count} replacements sent, {busy} records currently held. Unfinished attempts with unknown outcomes are excluded from the success rate.',
          records: 'Records and latest results (up to 50)', probes: 'Recent manual collection results (up to 50)', proxy: 'Proxy #{id}',
          results: '{success} successful and {failed} failed replacements', capturedAt: 'Last collected: {time}', expiresAt: 'Expires: {time}', usedAt: 'Last used: {time}',
          state: { available: 'Unexpired, ready to try', in_use: 'In use', expired: 'Expired', invalid: 'Unavailable' },
          outcome: { pending: 'Replacement outcome not yet confirmed', success: 'Last replacement succeeded', failed: 'Last replacement failed', decrypt_failed: 'Decryption failed; record disabled' }
        },
        testTitle: 'Collect healthy turn state manually',
        testConfigure: 'Choose proxies and model',
        testDesc: 'Use the saved account settings to send hi once through each selected route. Record only a model-matching, complete response with meaningful output, a state header and a first output within 5 seconds. Errors, including 429/503, are not recorded or retried. Manual collection works independently of the automatic recording switch and does not change the account proxy. Successful records enter the shared OpenAI pool and can be used by other accounts, models, routes and transports. Each test takes up to 60 seconds.',
