...
describe('AffiliateOfflineWithdrawDialog', () => {
  ...
  it('records the exact available quota as the withdrawal amount', async () => {
    ...
    expect(wrapper.get('[data-test="withdraw-available-quota"]').text()).toBe('12.34567891 灵石')
    ...
    expect(showSuccess).toHaveBeenCalledWith(
      'admin.affiliates.withdraw.success {"amount":"12.34567891 灵石","remaining":"0.00 灵石"}',
    )
  ...
  it('treats a user without an affiliate profile as having no available quota', async () => {
    ...
    expect(wrapper.get('[data-test="withdraw-available-quota"]').text()).toBe('0.00 灵石')
  ...
  it.each([
    ...
    expect(showSuccess).toHaveBeenCalledWith(
      'admin.affiliates.withdraw.replayed {"amount":"10.00 灵石","remaining":"0.00 灵石"}',
    )
  ...
  it('starts a new registration with a fresh key after a definite rejection', async () => {
    ...
    expect(showSuccess).toHaveBeenCalledWith('admin.affiliates.withdraw.success {"amount":"5.00 灵石","remaining":"5.00 灵石"}')
  })
})
