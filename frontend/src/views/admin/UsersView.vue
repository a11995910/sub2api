            ...
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                ...
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatSpiritStones(usageStats[row.id]?.today_actual_cost ?? 0, { fractionDigits: 4 }) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-gray-500 dark:text-gray-400">{{ t('admin.users.total') }}:</span>
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatSpiritStones(usageStats[row.id]?.total_actual_cost ?? 0, { fractionDigits: 4 }) }}
                </span>
              </div>
            </div>
          </template>
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <Icon name="gem" size="sm" class="text-emerald-500" :stroke-width="2" />
