        ...
        >
          ...
          <template #cell-name="{ value, row }">
            <div class="flex items-center gap-1.5">
              ...
              />
              <span
                v-if="row.openai_fast_mode_enabled"
                class="rounded bg-emerald-50 px-1.5 py-0.5 text-[10px] font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300"
                :title="t('keys.openaiFastModeEnabled')"
              >
                {{ t('keys.openaiFastModeBadge') }}
              </span>
            </div>
          </template>
            ...
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                ...
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatStoneAmount(usageStats[row.id]?.today_actual_cost ?? 0, 4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-gray-500 dark:text-gray-400">{{ t('keys.total') }}:</span>
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ formatStoneAmount(usageStats[row.id]?.total_actual_cost ?? 0, 4) }}
                </span>
              </div>
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  ...
                  <span :class="[
                    ...
                    {{ formatStoneAmount(row.quota_used || 0) }} / {{ formatStoneAmount(row.quota || 0) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ formatStoneAmount(row.usage_5h || 0) }}/{{ formatStoneAmount(row.rate_limit_5h || 0) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ formatStoneAmount(row.usage_1d || 0) }}/{{ formatStoneAmount(row.rate_limit_1d || 0) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ formatStoneAmount(row.usage_7d || 0) }}/{{ formatStoneAmount(row.rate_limit_7d || 0) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
    ...
    >
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="flex items-center justify-between gap-4 rounded-lg border border-gray-200 px-3 py-3 dark:border-dark-600">
          <div class="min-w-0">
            <label class="input-label mb-0">{{ t('keys.openaiFastMode') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('keys.openaiFastModeHint') }}</p>
          </div>
          <button
            type="button"
            role="switch"
            :aria-checked="formData.openai_fast_mode_enabled"
            :aria-label="t('keys.openaiFastMode')"
            @click="formData.openai_fast_mode_enabled = !formData.openai_fast_mode_enabled"
            :class="[
              'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
              formData.openai_fast_mode_enabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                formData.openai_fast_mode_enabled ? 'translate-x-4' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
        ...
        <div class="flex items-center justify-between gap-4 rounded-lg border border-gray-200 px-3 py-3 dark:border-dark-600">
          <div class="min-w-0">
            <label class="input-label mb-0">{{ t('keys.autoGroupFallback') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('keys.autoGroupFallbackHint') }}</p>
          </div>
          <button
            type="button"
            role="switch"
            :aria-checked="formData.auto_group_fallback_enabled"
            :aria-label="t('keys.autoGroupFallback')"
            @click="formData.auto_group_fallback_enabled = !formData.auto_group_fallback_enabled"
            :class="[
              'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
              formData.auto_group_fallback_enabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                formData.auto_group_fallback_enabled ? 'translate-x-4' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-4">
            <div>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-gray-500">{{ t('common.currencyName') }}</span>
                <input
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700">
                  <span class="font-medium text-gray-900 dark:text-white">
                    {{ formatStoneAmount(selectedKey.quota_used || 0, 4) }}
                  </span>
                  <span class="mx-2 text-gray-400">/</span>
                  <span class="text-gray-500 dark:text-gray-400">
                    {{ formatStoneAmount(selectedKey.quota || 0) }}
                  </span>
                </div>
                <button
                  type="button"
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ formatStoneAmount(selectedKey.usage_5h || 0, 4) }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ formatStoneAmount(selectedKey.rate_limit_5h || 0) }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ formatStoneAmount(selectedKey.usage_1d || 0, 4) }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ formatStoneAmount(selectedKey.rate_limit_1d || 0) }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ formatStoneAmount(selectedKey.usage_7d || 0, 4) }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ formatStoneAmount(selectedKey.rate_limit_7d || 0) }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
...
const formatStoneAmount = (value: number, fractionDigits = 2) =>
...
const editKey = (key: ApiKey) => {
  ...
  formData.value = {
    ...
    openai_fast_mode_enabled: !!key.openai_fast_mode_enabled,
    auto_group_fallback_enabled: key.auto_group_fallback_enabled !== false,
    use_custom_key: false,
    custom_key: '',
  ...
  try {
    if (showEditModal.value && selectedKey.value) {
      const updates: UpdateApiKeyRequest = {
        ...
        openai_fast_mode_enabled: formData.value.openai_fast_mode_enabled,
        auto_group_fallback_enabled: formData.value.auto_group_fallback_enabled,
      ...
      await keysAPI.create(
        ...
        expiresInDays,
        rateLimitData,
        formData.value.openai_fast_mode_enabled,
        formData.value.auto_group_fallback_enabled
      )
