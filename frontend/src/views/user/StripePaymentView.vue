  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <button class="btn btn-primary mt-6" @click="router.push('/payment')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        ...
        <template v-else-if="showPaymentElement">
          ...
          <div class="text-center">
            <button class="btn btn-secondary" @click="router.push('/payment')">{{ t('payment.result.backToRecharge') }}</button>
          </div>
        </template>
        ...
        <div v-if="stripeError && !showPaymentElement" class="card p-4">
          ...
          <button class="btn btn-secondary mt-3 w-full" @click="router.push('/payment')">{{ t('payment.result.backToRecharge') }}</button>
        </div>
      </template>
    </div>
  </component>
