        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="rounded-lg bg-green-100 p-2 dark:bg-green-900/30">
              <Icon name="gem" size="md" class="text-green-600 dark:text-green-400" />
            </div>
            <div class="min-w-0 flex-1">
              <p class="text-xs font-medium text-gray-500 dark:text-gray-400">
                {{ t('usage.totalCost') }}
          ...
          <template #cell-cost="{ row }">
            <div class="flex items-center gap-1.5 text-sm">
              <span class="font-medium text-green-600 dark:text-green-400">
                {{ formatCostValue(row.actual_cost, 6) }}
              </span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatCostValue(tooltipData.input_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.outputCost') }}</span>
              <span class="font-medium text-white">{{ formatCostValue(tooltipData.output_cost, 6) }}</span>
            </div>
            ...
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatCostValue(imageUnitPrice(tooltipData), 6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageTotalPrice') }}</span>
                <span class="font-medium text-white">{{ formatCostValue(tooltipData.total_cost || 0, 6) }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.unitPrice') }}</span>
              <span class="font-medium text-sky-300">{{ formatCostValue(tooltipData?.total_cost || 0, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">{{ formatCostValue(tooltipData.cache_creation_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ formatCostValue(tooltipData.cache_read_cost, 6) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            ...
            <span class="font-semibold text-green-400"
              >{{ formatCostValue(tooltipData?.actual_cost || 0, 6) }}</span
            >
          </div>
        </div>
        <!-- Tooltip Arrow (left side) -->
...
import { formatDateTime, formatDollarAmount, formatReasoningEffort, formatSpiritStones, formatTokenCountM } from '@/utils/format'
